...
- Public group QR entry also supports browser-bound account recovery and reverified continuation from another browser. Pre-registration distinguishes pending review from approved invitations. SMS links are copied for manual delivery; email supports configurable SMTP (including Gmail via STARTTLS) or Microsoft 365 Graph, with no church mailbox domain assumption. Deployment-only administrator activation/recovery provides a last-administrator fallback. Recovery replaces Passkeys while retaining existing sessions and member data. See [workflow and deployment configuration](docs/identity-access.md).
