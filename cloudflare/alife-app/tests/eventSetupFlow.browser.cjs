...
(async () => {
  ...
  try {
    for (const language of (process.env.ALIFE_QA_LANGUAGES || 'zh,en').split(',')) for (const width of (process.env.ALIFE_QA_WIDTHS || '320,768,1280').split(',').map(Number)) {
      ...
      const errors = [], creates = [], posterSaves = [], publishes = [], uploads = [], ai = [], detailSaves = [], acceptedPlans = [];
      ...
      for (const [moduleCode, en, cn] of [['PROGRAM.PRODUCTION', 'Programme and production', '节目与制作'], ['PLACE.RESOURCE', 'Venue and resources', '场地与资源']]) savedPlan.moduleDecisions.push({ ...structuredClone(proposal.moduleDecisions[0]), moduleCode, label: text(en, cn), status: 'selected', surfaceKey: moduleCode.toLowerCase() });
      const applySelections = input => ({ ...savedPlan, moduleDecisions: savedPlan.moduleDecisions.map(item => {
      ...
      await context.route('**/api/**', async route => {
        ...
        else if (pathname.endsWith('/workspace')) data = { eventId: 'qa-event', owningGroupId: 'qa-group', title: info.brief.title, canManage: true, items: [{ surfaceKey: 'team.work', moduleCode: 'TEAM.WORK', label: text('Team and tasks', '团队与任务'), order: 1, readiness: 'ready' }] };
        ...
        else if (pathname.endsWith('/team')) data = { members: [], roles: [], tasks: [], roleRequirements: [], readinessBlockers: [], canManage: true };
      ...
      await page.waitForURL('**/qa-event/workspace?flow=setup&stage=setup');
      await page.getByLabel(t('Choose a tool to configure', '选择要设置的功能')).waitFor(); assert.equal(creates.length, 1);
      ...
      assert.equal(await flow().getByRole('link', { name: /6.*Approval|6.*正式审批/ }).count(), 0);
      ...
      await page.getByLabel(t('Expected attendance', '所需人数'), { exact: true }).fill('35');
      await page.getByRole('group', { name: t('Session title', '环节名称'), exact: true }).getByLabel('English', { exact: true }).fill('Revised opening');
      ...
      assert.equal(await page.getByLabel(t('Venue capacity', '场地容量'), { exact: true }).inputValue(), '50');
      assert.equal(acceptedPlans[0].body.arrangements.sessions[0].id, 'qa-session'); assert.equal(acceptedPlans[0].body.arrangements.sessions[0].itemIds[0], 'qa-item'); assert.equal(acceptedPlans[0].body.arrangements.venueBookings[0].id, 'qa-booking');
      ...
      await page.getByRole('region', { name: t('Roles and shifts', '岗位与轮班'), exact: true }).getByLabel(t('People needed', '所需人数'), { exact: true }).fill('4');
      await checkLayout('optional-tools');
      await click('Review tool changes', '查看功能变更');
      await Promise.all([page.waitForResponse(response => response.url().endsWith('/plan/accept') && response.ok()), click('Confirm event arrangements', '确认保存活动安排')]);
      await page.getByRole('status').filter({ hasText: /Event arrangements saved|活动安排已保存/ }).waitFor();
      assert.equal(acceptedPlans.length, 2); assert.equal(acceptedPlans[1].body.arrangements.serviceSlots[0].id, 'qa-slot'); assert.equal(acceptedPlans[1].body.arrangements.serviceSlots[0].details.requiredCount, 4);
      ...
      if (width === 1280) {
        const picker = page.getByRole('combobox', { name: /Occurrence to arrange|安排场次/ });
        await picker.selectOption('qa-occurrence-2');
        const roster = page.getByRole('region', { name: t('Roles and shifts', '岗位与轮班'), exact: true });
        await page.waitForFunction(() => !document.querySelector('fieldset:disabled'));
        await roster.getByRole('button', { name: t('Add role / shift', '添加岗位／轮班'), exact: true }).click();
        const starts = roster.getByLabel(t('Start time', '开始时间'), { exact: true });
        assert.match(await starts.last().inputValue(), /^2026-10-11T/);
        await roster.getByRole('button', { name: t('Remove this slot', '移除此岗位'), exact: true }).last().click();
        await picker.selectOption('qa-occurrence');
        await page.waitForFunction(() => !document.querySelector('fieldset:disabled'));
      }
      if (process.env.ALIFE_QA_STOP_AFTER_ARRANGEMENTS === '1') { console.log(`PASS ${language} ${width}: shared details, roster/programme/venue edits, refreshed concurrency and selected occurrence defaults`); await context.close(); continue; }
      ...
      await stage(/6.*Approval|6.*正式审批/); await click('Generate a new version', '从当前资料生成新版本'); await submit(); await decide(false);
      ...
      await stage(/6.*Approval|6.*正式审批/); await stage(/7.*Poster|7.*海报制作/);
      ...
      assert.equal(published, false); assert.equal(await flow().getByRole('link', { name: /7.*Poster|7.*海报制作|8.*Publish|8.*发布活动/ }).count(), 0);
      await stage(/Details|活动资料/); await titleGroup().waitFor(); await stage(/6.*Approval|6.*正式审批/); await submit(); await decide(false);
