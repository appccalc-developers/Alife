...
(async () => {
  ...
  try {
    for (const language of ['zh', 'en']) for (const width of [375, 1280]) {
      ...
      await page.goto(`${base}${eventPath}/workspace/ram`, { waitUntil: 'domcontentloaded' });
      ...
      await page.waitForFunction(() => location.pathname.endsWith('/workspace') && !new URLSearchParams(location.search).has('tab'));
      ...
      assert.ok(requests.every(pathname => !pathname.includes('/workflow') && pathname !== '/api/event-workflow-templates'), requests.join(', '));
      assert.ok(requests.every(pathname => pathname !== '/api/events/extract' && !pathname.includes('/api/events/session/')), requests.join(', '));
      console.log(`PASS ${language} ${width}: embedded RAM save, workspace/preparation navigation, retired workflow redirect, no retired workflow or AI session API`);
      await context.close();
    }
