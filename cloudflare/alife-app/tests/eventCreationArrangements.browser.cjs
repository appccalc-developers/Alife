...
(async () => {
  ...
  try {
    for (const language of (process.env.ALIFE_QA_LANGUAGES || 'zh,en').split(',')) for (const width of (process.env.ALIFE_QA_WIDTHS || '320,375,768,1280').split(',').map(Number)) {
      ...
      assert.match(await page.locator('[data-event-details-card] .event-arrangement-date').textContent(), /2026-10-04 18:00 – 20:00/);
      assert.doesNotMatch(await page.locator('[data-event-details-card] .event-arrangement-date').textContent(), /2026-10-04 20:00/);
      await page.locator('[data-arrangement-tile="EVENT.DETAILS"]').click();
      assert.equal(await page.getByText(t('Back to modules', '返回模块总览'), { exact: true }).count(), 0);
      ...
      const open = async code => { const selecting = await tile(code).getAttribute('aria-expanded') !== 'true'; const beforeScroll = width >= 1024 ? await page.evaluate(() => window.scrollY) : null; if (selecting) await tile(code).click(); const root = page.locator(`[data-module-editor="${code}"]`); await root.waitFor({ state: 'visible' }); if (selecting) { await page.waitForFunction(code => document.activeElement === document.getElementById(`module-heading-${code}`), code); if (beforeScroll !== null) assert.equal(await page.evaluate(() => window.scrollY), beforeScroll, 'desktop module selection does not scroll'); } return root; };
      ...
      const enable = async code => { const root = await open(code); await work(root, 'Settings and responsibilities', '设置与职责'); const toggle = root.getByRole('switch'); assert.equal((await root.locator('summary').filter({ has: page.getByText(t('Settings and responsibilities', '设置与职责'), { exact: true }) }).textContent()).includes(t('Enabled', '已启用')), false); if (await toggle.getAttribute('aria-checked') !== 'true' && !await toggle.isDisabled()) await toggle.click(); return root; };
      ...
      await initialRoster.getByRole('switch').click();
      const team = await open('TEAM.WORK');
      assert.equal(await team.getByRole('switch').getAttribute('aria-checked'), 'true'); assert.equal(await team.getByRole('switch').isDisabled(), true);
      const money = await open('MONEY.FINANCE');
      await money.getByRole('switch').waitFor();
      ...
      await money.locator('details[data-tool-panel]:visible summary').click();
      ...
      const settingsCard = settingsSummary.locator('..');
      assert.equal(await settingsCard.getAttribute('open') !== null, true, 'settings opens initially');
      ...
      assert.equal(await settingsCard.getAttribute('open'), null);
      ...
      await work(ram, 'Risk details', '风险明细');
      await ram.getByRole('switch').click();
      await ram.getByRole('switch').click();
