...
test('website content moves out of sidebars and group management has no pending badge', async () => {
  ...
  assert.match(groupItems, /getGroupLifeMemberships\(auth.memberships, churchGroupId, groupDirectory.data \?\? \[\]\)/)
...
test('standalone group management sections remain distinct from the default group section', () => {
  ...
  assert.equal(normalizeChurchManagementSection('ministries'), 'ministries')
  assert.equal(resolveManageSection('subgroups', ['group', 'subgroups', 'ministries']), 'subgroups')
  assert.equal(resolveManageSection('ministries', ['group', 'subgroups', 'ministries']), 'ministries')
  assert.equal(resolveManageSection('ministries', ['group', 'subgroups']), 'group')
  assert.equal(resolveManageSection('ministries', undefined, false), 'subgroups')
  assert.equal(resolveManageSection('ministries', undefined, true), 'ministries')
