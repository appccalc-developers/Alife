  ...
  const accept = async () => {
    ...
    try {
      const created = await eventService.createGroupEvent(groupId, creationEvent(draft, type, me?.displayName || ''), {
        ...
        initialRamDraft: ramTouched ? { ...ramDraft, ...(draft.activityPlan || emptyActivityPlan()) } : undefined,
      })
