...
import { Check, LogOut, Mail, Pencil, Phone, ShieldCheck, SlidersHorizontal, UserRound, X } from 'lucide-react'
...
const ProfileView = () => {
  ...
  const copy = language === 'zh'
    ? {
      ...
      accountStatus: '账号状态', settings: '带领人设置', signOutSubtitle: '退出后，需要重新登录才能使用会员功能。',
    }
    : {
      ...
      accountStatus: 'Account status', settings: 'Leader settings', signOutSubtitle: 'You will need to sign in again to use member features.',
    }
  ...
  const hasLeaderUiOptions = me.memberships.some(
    (membership) => membership.status === 'approved' && (membership.role === 'leader' || membership.role === 'coLeader'),
  )
    ...
    <AppPageShell>
      <section className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[#123f35] via-[#176b5a] to-[#24917a] px-5 py-6 text-white shadow-[0_20px_55px_rgba(18,63,53,0.24)] sm:px-8 sm:py-8">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          {!editing && !me.isGuest ? (
            <AppActionButton className="border-white/30 bg-white/95 text-[#0d4f43] hover:bg-white" onClick={beginEditing}>
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1.45fr)_minmax(18rem,0.75fr)]">
        <AppSectionCard title={copy.contactTitle} subtitle={copy.contactSubtitle}>
          {editing ? (
        ...
        <div className="space-y-5">
          <AppSectionCard dense title={copy.accountStatus}>
            <div className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 p-3">
              <span className="text-sm font-semibold text-slate-700">{t('status')}</span>
              <AppBadge variant={me.isRegistered ? 'success' : 'neutral'}>{me.isRegistered ? t('registered') : t('guest')}</AppBadge>
            </div>
          </AppSectionCard>
          {hasLeaderUiOptions ? (
            <AppSectionCard dense title={copy.settings} subtitle={t('profileLeaderUiSubtitle')}>
              <label className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                <input type="checkbox" className="mt-0.5 h-4 w-4 rounded border-slate-300 text-emerald-700 focus:ring-emerald-500" checked={leaderUiPreferences.exerciseGroupManagement} onChange={(event) => setLeaderUiPreferences((current) => ({ ...current, exerciseGroupManagement: event.target.checked }))} />
                <span><span className="flex items-center gap-2 text-sm font-semibold text-slate-900"><SlidersHorizontal className="h-4 w-4" />{t('exerciseGroupManagement')}</span><span className="mt-1 block text-xs leading-5 text-slate-600">{t('exerciseGroupManagementHelp')}</span></span>
              </label>
            </AppSectionCard>
          ) : null}
        </div>
