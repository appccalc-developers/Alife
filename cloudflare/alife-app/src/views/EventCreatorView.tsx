...
const LegacyEventEditor = () => {
  ...
  const activeIds = useActiveEntityIds({
    ...
    eventId: routeEventId || searchParams.get('eventId') || undefined,
  })
  ...
  useEffect(() => {
    if (isEditMode || !targetEventId || !eventDraft?.ram || !ramHasLocalChanges || ramBusy || ramAutosaveInFlight || ramAutosaveFailedJsonRef.current === ramDraftJson) {
      return
    }
    ...
    try {
      ...
      if (response.responseMode === 'result' && response.result) {
        ...
        setEventDraft((current) => ({
          ...
          ram: current?.ram ?? createEmptyEventRamDraft(dto),
        }))
        setSaveStatus('idle')
  ...
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-5">
      {effectiveGroupId && eventId ? <div className="flex flex-wrap gap-4 text-sm font-semibold text-[#176b5a]">
        <Link to={searchParams.get('flow') === 'setup'
          ? `${buildScopedEventDetailPath(effectiveGroupId, eventId, Boolean(routeGroupId || searchParams.get('groupId')))}/workspace?flow=setup&stage=arrangements&module=safety.ram`
          : buildScopedEventDetailPath(effectiveGroupId, eventId, Boolean(routeGroupId || searchParams.get('groupId')))}>
          {searchParams.get('flow') === 'setup' ? (language === 'zh' ? '← 返回活动筹备' : '← Back to event preparation') : (language === 'zh' ? '← 返回活动' : '← Back to event')}
        </Link>
        <Link to={`${buildScopedEventDetailPath(effectiveGroupId, eventId, Boolean(routeGroupId || searchParams.get('groupId')))}?section=workflow`}>
      ...
      <div id="event-editor-panel-ram" role="tabpanel" aria-labelledby="event-editor-tab-ram" hidden={activeTab !== 'ram'} className="space-y-5">
        {targetEventId ? <EventRamWorkspace key={targetEventId} eventId={targetEventId} language={language} /> : <>
        <SubmissionStatusHeader
          title={language === 'zh' ? '风险评估与管理' : 'Risk Assessment and Management'}
          description={language === 'zh' ? '请先建立活动，再进入新版 RAM 填写、本人确认和独立审核。' : 'Create the event first, then complete versioned RAM, personal confirmation and independent review.'}
