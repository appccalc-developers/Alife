...
const PersonApplicationsPanel = ({ language }: { language: string }) => {
  ...
  useEffect(() => { load().catch((caught) => setError(identityWorkflowError(caught, language))) }, [load])
  ...
  const decide = async (application: MembershipApplication, decision: 'approved' | 'needsInfo' | 'rejected') => {
    ...
    setError('')
    try {
      await identityAccessService.decidePersonApplication(application, decision, note || undefined, links[application.id]?.trim(), application.isIdentityVerified || verified[application.id] === true)
      await load()
    } catch (caught) {
  ...
  return (
    <details className="border-b border-[#dce7e2] bg-[#f7faf8] px-4 py-4 sm:px-6">
      ...
      {items.length ? <div className="mt-4 space-y-3">{items.map((application) => <article key={application.id} className="rounded-xl border border-[#dce7e2] bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-bold text-[#18332d]">{application.displayName}</p><p className="text-xs">{application.source === 'recoveryQr' ? (zh ? '恢复帐号' : 'Account recovery') : application.source === 'continuationQr' ? (zh ? '继续原申请' : 'Continue an application') : ''}</p><p className="mt-1 text-xs text-[#687770]">{application.id} · {application.maskedPhone} · {application.groupNameEn || application.groupNameZh}</p></div><div className="flex gap-2"><span className="rounded-full bg-[#e3f0eb] px-2 py-1 text-xs font-bold text-[#176b5a]">{application.personStatus}</span><span className="rounded-full bg-[#f1eee7] px-2 py-1 text-xs font-bold text-[#6e655b]">{application.matchState}</span></div></div>{application.responseDeliveryStatus === 'unavailable' || application.responseDeliveryStatus === 'failed' ? <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-900" role="status">{copy.deliveryUnavailable}</p> : null}{!['approved', 'rejected'].includes(application.personStatus) ? <div className="mt-3 grid gap-2 sm:grid-cols-2"><label className="text-xs font-bold text-[#62736c]">{copy.note}<textarea className="alife-input mt-1 min-h-20 py-2" value={notes[application.id] ?? ''} onChange={(event) => setNotes((current) => ({ ...current, [application.id]: event.target.value }))} /></label>{['recoveryQr', 'continuationQr'].includes(application.source) || application.matchState === 'possible' || application.matchState === 'ambiguous' ? <label className="text-xs font-bold text-[#62736c]">{application.source === 'continuationQr' ? (zh ? '原申请编号' : 'Original application reference') : copy.linked}{application.source === 'recoveryQr' ? <RecoveryMemberSelect groupId={application.groupId} zh={zh} value={links[application.id] ?? ''} onChange={value => setLinks(current => ({ ...current, [application.id]: value }))} /> : <input className="alife-input mt-1" value={links[application.id] ?? ''} onChange={(event) => setLinks((current) => ({ ...current, [application.id]: event.target.value }))} />}</label> : null}<label className="flex items-center gap-2 text-xs font-bold text-[#62736c] sm:col-span-2"><input type="checkbox" className="h-4 w-4 accent-[#176b5a]" checked={application.isIdentityVerified || verified[application.id] === true} disabled={application.isIdentityVerified} onChange={(event) => setVerified((current) => ({ ...current, [application.id]: event.target.checked }))} />{copy.verified}</label><div className="flex flex-wrap gap-2 sm:col-span-2"><button className="min-h-9 rounded-lg bg-[#176b5a] px-3 text-xs font-black text-white disabled:opacity-50" type="button" disabled={busy === application.id || (!application.isIdentityVerified && verified[application.id] !== true)} onClick={() => void decide(application, 'approved')}>{copy.approve}</button><button className="min-h-9 rounded-lg border border-[#cbdad4] px-3 text-xs font-black disabled:opacity-50" type="button" disabled={busy === application.id || !notes[application.id]?.trim()} onClick={() => void decide(application, 'needsInfo')}>{copy.info}</button><button className="min-h-9 rounded-lg border border-rose-200 px-3 text-xs font-black text-rose-700 disabled:opacity-50" type="button" disabled={busy === application.id} onClick={() => void decide(application, 'rejected')}>{copy.reject}</button></div></div> : null}</article>)}</div> : <p className="mt-4 text-sm text-[#687770]">{copy.empty}</p>}
