...
const IdentityLinkEntryView = () => {
  ...
  const continueFlow = async () => {
    ...
    try {
      if (kind === 'activation') {
        await identityAccessService.resolveActivation(selector, secret, false)
  ...
  return (
    <section className="mx-auto flex min-h-[calc(100dvh-8rem)] w-full max-w-xl items-center px-4 py-8">
      <div className="w-full rounded-[2rem] border border-[#2f4b42]/10 bg-[#fffdf8]/95 p-6 shadow-[0_24px_70px_rgba(31,56,48,0.12)] sm:p-9">
        ...
        {kind === 'activation' && !mobileDevice ? <p className="mt-6 rounded-2xl border border-[#e37b63]/25 bg-[#fff2ed] px-4 py-3 text-sm leading-6 text-[#915040]" role="alert">{t('activationMobileRequired')}</p> : null}
        <button className="alife-primary-button mt-6 w-full" type="button" disabled={busy || (kind === 'activation' && !mobileDevice)} onClick={() => void continueFlow()}>
