...
const MinistrySection = ({ copy, language, pages }: Props) => {
  ...
  const activePath = activeCard ? publicPageHomePath(activeCard, language) : ''

  const cardImageUrl = useCallback((page: PageSummaryDto, index: number) =>
    page.cardImageUrl?.trim() || fallbackGroupImages[index % fallbackGroupImages.length],
  [])
        ...
        <motion.div {...entrance} className="mt-10 overflow-hidden rounded-2xl bg-home-dark text-white shadow-[0_28px_80px_rgba(34,25,17,0.18)]">
          <div className="relative min-h-[31rem]">
            <AnimatePresence initial={false} custom={direction} mode="wait">
              ...
              >
                <Link to={activePath} className="group block h-full">
                  <img
                    src={cardImageUrl(activeCard, activeIndex)}
                    alt={publicPageMenuName(activeCard, language)}
                ...
                return (
                  ...
                  >
                    <div className="h-16 overflow-hidden rounded-lg bg-white/10">
                      <img src={cardImageUrl(card, index)} alt="" className="h-full w-full object-cover opacity-80 transition group-hover:scale-[1.04]" loading="lazy" />
                    </div>
                    <p className="mt-2 line-clamp-1 text-xs font-semibold text-white">{publicPageMenuName(card, language)}</p>
