...
export const EventTravelWorkspace = ({ eventId, eventBasePath, language, setupFlow, onBusyChange, onSaved }: EventSurfaceProps) => {
  ...
  if (loadState === 'loading') return <AppSectionCard summary={language === 'zh' ? '正在读取交通资料…' : 'Loading travel data…'} title={language === 'zh' ? '交通与乘车名单' : 'Travel & passenger manifest'}><p role="status" className="text-sm text-[#66766f]">{language === 'zh' ? '正在载入受限交通资料…' : 'Loading restricted travel data…'}</p></AppSectionCard>
...
const DriverCard = ({ driver, language, eventId, busy, mutate }: { driver: EventTravelDriver; language: Language; eventId: string; busy: boolean; mutate: Mutate }) => {
  ...
  useArrangementDraft(expiry !== (driver.licenceExpiresOn ?? '') || licenceConfirmed !== driver.licenceConfirmed || fit !== driver.fitToDriveConfirmed)
...
const VehicleCard = ({ vehicle, language, eventId, busy, mutate }: { vehicle: EventTravelVehicle; language: Language; eventId: string; busy: boolean; mutate: Mutate }) => {
  ...
  useArrangementDraft(seats !== vehicle.seatCapacity || regoExpiry !== (vehicle.registrationExpiresOn ?? '') || wofExpiry !== (vehicle.wofExpiresOn ?? ''))
...
const JourneyPanel = ({ data, language, busy, mutate }: PanelProps) => {
  ...
  const journeySaved = useArrangementFormDraft([occurrenceId, nameEn, nameZh, start, end])
...
const JourneyCard = ({ journey, data, language, busy, mutate }: { journey: EventTravelJourney; data: EventTravelWorkspaceData; language: Language; busy: boolean; mutate: Mutate }) => {
  ...
  useArrangementDraft(driverId !== (journey.driver?.id ?? '') || vehicleId !== (journey.vehicle?.id ?? '') || confirmed !== journey.manifestConfirmed || status !== journey.status)
  const stopSaved = useArrangementFormDraft([stopEn, stopZh, stopAddress, pickup])
  const passengerSaved = useArrangementFormDraft([passengerId, pickupStopId])
  ...
  const addStop = (event: FormEvent) => { event.preventDefault(); void mutate(() => eventTravelService.addStop(data.eventId, journey.id, journey.eTag, { sortOrder: journey.pickupStops.length, name: { en: stopEn, zh: stopZh }, address: { en: stopAddress, zh: stopAddress }, pickupUtc: toUtc(pickup) }).then(stopSaved), language === 'zh' ? '接送点已加入。' : 'Pickup stop added.') }
  const assign = (event: FormEvent) => { event.preventDefault(); void mutate(() => eventTravelService.assignPassenger(data.eventId, journey.id, journey.eTag, passengerId, pickupStopId).then(passengerSaved), language === 'zh' ? '乘客已加入受限名单。' : 'Passenger added to the restricted manifest.') }
...
const StopRow = ({ stop, journey, eventId, language, busy, mutate }: { stop: EventTravelPickupStop; journey: EventTravelJourney; eventId: string; language: Language; busy: boolean; mutate: Mutate }) => {
  ...
  useArrangementDraft(pickup !== toInput(stop.pickupUtc))
