...
import { assistantApplicableFields, assistantFieldComplete, formAssistantFields, validateFormAssistantResult, type AssistantForm, type FormAssistantContext, type FormAssistantInput, type FormAssistantScope } from '../../../../../shared/eventFormAssistant'
...
export default function EventFormAssistant({ eventId, scope, form, context, contextSignature, zh, active, initiallyConfirmed = false, onAdopt, onBusy }: {
  ...
  const send = async (message: string, turns: AssistantTurn[]) => {
    ...
    const input: FormAssistantInput = { eventId, scope, form, context, revision: revision.current, language: zh ? 'zh' : 'en', timeZone, message, history: turns.slice(-8).map(turn => ({ role: turn.role, text: typeof turn.text === 'string' ? turn.text : turn.text[zh ? 'zh' : 'en'] })) }
  ...
  return <EventAssistantChat zh={zh} active={active} readOnly={readOnly} onBusy={value => { setBusy(value); onBusy?.(value) }} onSend={send}
    heading={<div className="event-assistant-heading"><h3>{zh ? 'AI 资料助手' : 'AI details assistant'}</h3><span>{scope === 'tasks' ? (zh ? '任务与交接' : 'Tasks and handoffs') : scope === 'registration' ? (zh ? '邀请报名' : 'Registration') : (zh ? '活动项目与条件' : 'Activities and conditions')}</span></div>}
    intro={scope === 'tasks' ? (zh ? '说说这项任务要做什么、何时完成，以及是否需要审核。负责人、审核人和场次请在表单中选择。' : 'Describe this task, its deadline and whether review is needed. Select people and the occurrence in the form.') : scope === 'registration' ? (zh ? '说说为什么需要报名、参加条件、人数与时间安排。我们也可以一起整理双语条款、材料要求和费用说明。请勿输入参加者隐私或支付凭据。' : 'Describe the registration purpose, eligibility, capacity and timing. We can also organise bilingual terms, material requirements and fee instructions. Keep participant details and payment credentials out of the message.') : (zh ? '从活动名称和说明中整理活动项目与条件，协助判断室外／场外、过夜和高风险情况。AI 只填入候选草稿，请逐项核对后保存；不要输入个人、医疗或联络资料。' : 'Use the event title and description to draft activities and conditions, and suggest outdoor/off-site, overnight and high-risk flags. AI only fills a candidate draft; review each field before saving. Do not enter personal, medical or contact details.')}
