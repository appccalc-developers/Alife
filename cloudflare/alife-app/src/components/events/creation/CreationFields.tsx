...
export function BilingualField({ label, value, onChange, multiline, primaryLanguage, required = false, maxLength }: { label: string; value: MultilingualString; onChange: (value: MultilingualString) => void; multiline?: boolean; primaryLanguage?: 'en' | 'zh'; required?: boolean; maxLength?: number }) {
  if (primaryLanguage) {
    ...
    const control = (code: 'en' | 'zh') => <Field label={`${label} · ${code === 'zh' ? '中文' : 'English'}`}>{multiline
      ? <textarea rows={4} data-locale={code} className={`${creationInput} py-2`} required={required} maxLength={maxLength} value={value[code]} onChange={e => onChange({ ...value, [code]: e.target.value })} />
      : <input data-locale={code} className={creationInput} required={required} maxLength={maxLength} value={value[code]} onChange={e => onChange({ ...value, [code]: e.target.value })} />}</Field>
