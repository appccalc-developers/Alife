...
const DetailsContext = createContext({
  readOnly: false,
  updatedFields: [] as DetailField[],
  revealField: (_field?: DetailField) => {},
  markUpdated: (_fields: DetailField[]) => {},
})
export const useDetailsWorkspace = () => useContext(DetailsContext)

export default function DetailsWorkspace({ zh, active, form, assistant, readOnly = false, focusRequest }: {
  ...
  const [updatedFields, setUpdatedFields] = useState<DetailField[]>([])
  const [pendingReveal, setPendingReveal] = useState<{ field?: DetailField; version: number }>()
  useEffect(() => {
  ...
  const revealField = useCallback((field?: DetailField) => {
    setView('form')
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const region = field ? formRegion.current?.querySelector<HTMLElement>(`[data-detail-field="${field}"]`) : formRegion.current
      if (!region) return
  ...
  return <DetailsContext.Provider value={{ readOnly, updatedFields, revealField, markUpdated: setUpdatedFields }}>
    <div ref={root} className="event-details-workspace" data-wide={wide} data-assistant-open={assistantOpen}>
      {!wide ? <div className="event-details-tabs" role="tablist" aria-label={zh ? '活动资料工作区' : 'Event details workspace'}>
        ...
        }}>{tab === 'form' ? <ClipboardList size={18} aria-hidden="true" /> : <MessagesSquare size={18} aria-hidden="true" />}{tab === 'form' ? (zh ? '资料表单' : 'Details form') : (zh ? 'AI 助手' : 'AI assistant')}</button>)}
      ...
      <div className="event-details-columns">
        <div ref={formRegion} id={`${id}-form`} role={wide ? 'region' : 'tabpanel'} aria-label={wide ? (zh ? '资料表单' : 'Details form') : undefined} aria-labelledby={wide ? undefined : `${id}-form-tab`} tabIndex={-1} hidden={!wide && view !== 'form'} className="event-details-form">
          {!wide && readOnly ? <p className="event-details-readonly">{zh ? '只读 · 可浏览全部资料' : 'Read-only · browse all details'}</p> : null}{form}
        </div>
        <div id={`${id}-assistant`} role={wide ? 'complementary' : 'tabpanel'} aria-label={wide ? (zh ? 'AI 资料助手' : 'AI details assistant') : undefined} aria-labelledby={wide ? undefined : `${id}-assistant-tab`} hidden={wide ? !assistantOpen : view !== 'assistant'} className="event-details-assistant">
