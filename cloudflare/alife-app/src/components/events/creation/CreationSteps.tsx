...
export function DetailsStep({ draft, setDraft, zh, type, archetype, ai, saved = false }: DraftProps & { type: EventActivityType; archetype: EventArchetype; ai: ReactNode; saved?: boolean }) {
...
function ArrangementModule({ draft, setDraft, zh, decision, current, type, groupId, saved = false }: DraftProps & { decision: ModuleDecision; current: boolean; type: EventActivityType; groupId: string; saved?: boolean }) {
  ...
  const editor = decision.moduleCode === 'SERVICE.ROSTER' ? <CreationRosterEditor {...editorProps} /> : decision.moduleCode === 'PROGRAM.PRODUCTION' ? <CreationProgrammeEditor {...editorProps} /> : decision.moduleCode === 'PLACE.RESOURCE' ? <CreationVenueEditor {...editorProps} /> : null
  const planningOnly = ['MONEY.FINANCE', 'FOOD.HOSPITALITY', 'FESTIVAL.OPERATIONS'].includes(decision.moduleCode)
  return <section aria-label={localText(decision.label, zh)} className="rounded-xl bg-[#f5f2eb] p-3 text-sm">
    <div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-semibold">{localText(decision.label, zh)}</h3>{required ? <AppBadge variant="warning">{zh ? '必需' : 'Required'}</AppBadge> : <div role="group" aria-label={`${localText(decision.label, zh)} · ${zh ? '是否启用' : 'Enable'}`} className="flex gap-2">{[true, false].map(value => <button key={String(value)} type="button" disabled={!current} aria-pressed={selected === value} className={`${choice(selected === value)} min-w-16 disabled:opacity-50`} onClick={() => { setDraft(previous => saved ? { ...previous, moduleOverrides: { ...previous.moduleOverrides, [decision.moduleCode]: value } } : changeOptionalModule(previous, decision, value)); if (value) setExpanded(true) }}>{value ? (zh ? '是' : 'Yes') : (zh ? '否' : 'No')}</button>)}</div>}</div>
    ...
    {selected && editor ? <details open={expanded} onToggle={event => setExpanded(event.currentTarget.open)} className="mt-3"><summary className="min-h-11 cursor-pointer py-2 font-semibold text-[#176b5a]">{zh ? '编辑具体安排' : 'Edit arrangements'}</summary>{editor}</details> : null}
  </section>
}

export function ArrangementsStep({ draft, setDraft, zh, type, proposal, current, status, groupId, saved = false }: DraftProps & { type: EventActivityType; proposal: EventPlanProposal | null; current: boolean; status: ReactNode; groupId: string; saved?: boolean }) {
  const issue = current && proposal ? validateCreationArrangements(draft, type, proposal, zh) : ''
  return <div className="space-y-4">{status}<p className="text-sm text-[#66766f]">{zh ? '确认本次活动的实际情况，直接选择并展开布置所需功能。暂不清楚的事实请保留“待确认”。' : 'Confirm the event facts, choose the tools you need, and expand them to plan details here. Keep uncertain facts pending.'}</p>
    ...
    {arrangementGroups.map(group => <AppSectionCard key={group.en} title={zh ? group.zh : group.en}>
      <div className="grid gap-3 md:grid-cols-2">{group.facts.map(([code, en, cn]) => <fieldset key={code} className="min-w-0"><legend className="text-sm font-semibold">{zh ? cn : en}</legend>{code === 'safety.requiresRam' ? <p className="mt-1 text-xs leading-5 text-[#66766f]">{zh ? '依据负责人或现行安全要求确认；不清楚时保留待确认。选择“否”不代表已通过安全审批。' : 'Confirm with the organiser or applicable safety requirements. Keep pending if unsure. “No” is not safety approval.'}</p> : null}<div className="mt-2 grid grid-cols-3 gap-2">{(['unknown', 'yes', 'no'] as const).map(value => <button key={value} type="button" className={choice(draft.factValues[code] === value)} aria-pressed={draft.factValues[code] === value} onClick={() => setDraft(previous => ({ ...previous, factValues: { ...previous.factValues, [code]: value } }))}>{value === 'unknown' ? (zh ? '待确认' : 'Pending') : value === 'yes' ? (zh ? '是' : 'Yes') : (zh ? '否' : 'No')}</button>)}</div>{draft.factValues[code] === 'unknown' && code in draft.aiCandidateFacts ? <p className="mt-1 text-xs text-amber-800">{zh ? 'AI 建议（未确认）：' : 'AI suggestion (unconfirmed): '}{draft.aiCandidateFacts[code] ? (zh ? '是' : 'Yes') : (zh ? '否' : 'No')}</p> : null}</fieldset>)}</div>
      <div className="mt-3 space-y-3">{proposal?.moduleDecisions.filter(item => group.modules.some(code => code === item.moduleCode)).map(decision => <ArrangementModule key={decision.moduleCode} {...{ draft, setDraft, zh, type, decision, current, groupId, saved }} />)}</div>
    </AppSectionCard>)}
    ...
    {current && proposal ? <><CreationArrangementSummary {...{ draft, type, proposal, zh }} />{issue ? <p role="status" className="rounded-xl bg-amber-50 p-3 text-sm text-amber-900">{issue}</p> : <p role="status" className="text-sm text-[#176b5a]">{zh ? '岗位、节目及场地安排已填写，可以继续确认。' : 'Roles, programme and venue details are complete and ready for review.'}</p>}</> : null}
  </div>
}

export function ReviewStep({ draft, zh, type, archetype, proposal, saved = false }: { draft: CreationDraft; zh: boolean; type: EventActivityType; archetype: EventArchetype; proposal: EventPlanProposal; saved?: boolean }) {
  const settings = creationSettings(draft, type)
