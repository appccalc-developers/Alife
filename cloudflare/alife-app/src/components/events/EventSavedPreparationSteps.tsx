  ...
  return <div hidden={!['details', 'arrangements', 'review'].includes(stage)} className="space-y-4">
    ...
    {record ? <fieldset ref={form} disabled={busy || ramBusy || anyToolBusy || loading || readOnly || aiBusy || planChanged || reloadRequired} tabIndex={-1} className="min-w-0 scroll-mt-24 space-y-4 outline-none">
      ...
      {arrangementsVisited ? <div hidden={stage !== 'arrangements'}>
        ...
        <ArrangementsStep {...{ setDraft, zh, type, groupId }} draft={data ? { ...draft, startLocal: localPreparationDate(data.startUtc, draft.timeZone), endLocal: localPreparationDate(data.endUtc, draft.timeZone) } : draft} saved ownerPanel={rolePanel()} rolePanels={Object.fromEntries((proposal?.moduleDecisions ?? []).map(item => [item.moduleCode, rolePanel(item.moduleCode)]))} focusModule={focusModule} modulePanels={Object.fromEntries(workspaceItems.filter(item => !item.surfaceKey.startsWith('workspace.') && item.surfaceKey !== 'safety.ram' && resolveEventSurface(item.surfaceKey)).map(item => [item.surfaceKey.toUpperCase(), <EventSurfaceRenderer key={item.surfaceKey} item={item} eventId={eventId} groupId={groupId} eventBasePath={eventBasePath} canManage={!readOnly} language={zh ? 'zh' : 'en'} setupFlow onBusyChange={value => setToolBusy(previous => previous[item.surfaceKey] === value ? previous : { ...previous, [item.surfaceKey]: value })} onSaved={() => toolSaved(item.surfaceKey.toUpperCase())} />]))} ramDirty={ramDirty} ramPanel={<EventRamWorkspace eventId={eventId} language={zh ? 'zh' : 'en'} onDirtyChange={value => { setRamDirty(value); if (value) setDraft(previous => previous.arrangementConfirmations?.safety ? invalidateArrangementConfirmation(previous, 'SAFETY.RAM') : previous) }} onBusyChange={setRamBusy} onSaved={ramSaved} pendingEventChanges={detailsDirty || arrangementsDirty || reloadRequired} />} proposal={review?.proposal ?? plan?.plan ?? null} current={Boolean(current && !previewing)} status={<p role="status" className="text-sm text-[#66766f]">{previewing ? (zh ? '正在更新活动方案……' : 'Updating the event plan…') : (zh ? '各项功能均可选是或否；关闭会保留原有资料。' : 'Every tool has a Yes/No choice; disabling keeps saved details.')}</p>} />
      </div> : null}
