...
export default function EventSavedPreparationSteps({ eventId, groupId, eventBasePath, workspaceItems, focusModule, plan, archetypes, stage, zh, readOnly, onBusy, onDirty, onSaved, go }: {
  ...
  const canAutoSaveArrangements = stage === 'arrangements' && Boolean(record) && !readOnly && !unsavedTools && !Object.values(moduleDirty).some(Boolean) && !ramDirty && !ramBusy && !anyToolBusy && !busy && !loading && !previewing && !reloadRequired && !planChanged && review !== null && Boolean(current) && !detailsDirty && !detailsValidation
