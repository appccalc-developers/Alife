...
export default function EventSavedPreparationSteps({ eventId, groupId, eventBasePath, workspaceItems, focusModule, plan, archetypes, stage, zh, readOnly, onBusy, onDirty, onSaved, go }: {
  ...
  const content = <div hidden={!['details', 'arrangements', 'review'].includes(stage)} className="space-y-4">
    ...
    {record && !readOnly && ['details', 'arrangements'].includes(stage) ? <div className="flex flex-wrap items-center gap-2" role="group" aria-label={zh ? '保存筹备草稿' : 'Save preparation draft'}>
      ...
      <AppActionButton disabled={!canSaveArrangements} onClick={() => void saveArrangements()}>{zh ? '保存活动安排' : 'Save arrangements'}</AppActionButton>
