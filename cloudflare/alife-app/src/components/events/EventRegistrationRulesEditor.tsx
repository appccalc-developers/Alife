...
export default function EventRegistrationRulesEditor({ eventId, onDirty, onSaved }: { eventId: string; onDirty?: (dirty: boolean) => void; onSaved?: () => void }) {
  ...
  return <AppSectionCard title={zh ? '报名规则与办理程序' : 'Registration rules and procedures'} subtitle={zh ? '这里制定规则。邀请、名单、材料和收款在报名工作空间处理。' : 'Define rules here. Invitations, participants, materials and receipts are handled in the registration workspace.'}>
    ...
    <form onSubmit={e => { e.preventDefault(); void save() }}><fieldset disabled={busy || !query.data.canConfigure} className="space-y-5">
      <BilingualRuleField label={zh ? '为什么需要报名' : 'Why registration is needed'} value={draft.purpose} onChange={v => update('purpose', v)} required />
      <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm">{zh ? '报名资格' : 'Eligible participants'}<select className={inputStyle} value={draft.audience} onChange={e => update('audience', e.target.value)}>{[['invited', '仅点名邀请', 'Named invitations'], ['group', '指定小组成员', 'Selected group members'], ['church', '本教会成员', 'Church members'], ['public', '符合公开范围的用户', 'Users within event visibility']].map(([v, cn, en]) => <option key={v} value={v}>{zh ? cn : en}</option>)}</select></label>
      ...
      <BilingualRuleField label={zh ? '附加资格条件' : 'Additional eligibility conditions'} value={draft.eligibility} onChange={v => update('eligibility', v)} />
      <div className="grid gap-4 sm:grid-cols-3"><label className="grid gap-2 text-sm">{zh ? '人数上限（实际参加人数）' : 'Capacity (actual people)'}<input className={inputStyle} type="number" min={1} max={1000000} required value={draft.capacity} onChange={e => update('capacity', Number(e.target.value))} /></label>{(['opensUtc', 'deadlineUtc'] as const).map(k => <label key={k} className="grid gap-2 text-sm">{k === 'opensUtc' ? zh ? '报名开放时间' : 'Opens' : zh ? '报名截止时间' : 'Deadline'}<input className={inputStyle} type="datetime-local" required value={localDateTime(draft[k])} onChange={e => e.target.value && update(k, new Date(e.target.value).toISOString())} /></label>)}</div>
      ...
      <label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={draft.allowWaitlist} onChange={e => update('allowWaitlist', e.target.checked)} />{zh ? '满额后允许按顺序候补' : 'Allow a first-in-first-out waitlist'}</label>
      {(['terms', 'privacyNotice', 'cancellationTerms'] as const).map((k, i) => <BilingualRuleField key={k} label={(zh ? ['参加规则', '隐私说明', '取消条款'] : ['Participation rules', 'Privacy notice', 'Cancellation terms'])[i]} value={draft[k]} onChange={v => update(k, v)} required />)}
      <label className="grid gap-2 text-sm">{zh ? '办理渠道' : 'Registration channel'}<select className={inputStyle} value={draft.channel} onChange={e => update('channel', e.target.value)}>{[['app', 'App 内办理', 'In app'], ['manual', '人工登记', 'Manual entry'], ['both', '两者并用', 'Both']].map(([v, cn, en]) => <option key={v} value={v}>{zh ? cn : en}</option>)}</select></label>
