...
import { EventToolSection as AppSectionCard, useArrangementFormDraft } from './ArrangementTileDeck'
...
export const EventSafeguardingWorkspace = ({ eventId, language, onBusyChange, onSaved }: EventSurfaceProps) => {
  ...
  if (loadState === 'loading') return <AppSectionCard summary={language === 'zh' ? '正在读取儿童保护资料…' : 'Loading safeguarding data…'} title={language === 'zh' ? '儿童保护签到' : 'Child safeguarding check-in'}><p role="status" className="text-sm text-[#66766f]">{language === 'zh' ? '正在载入受限儿童保护资料…' : 'Loading restricted safeguarding data…'}</p></AppSectionCard>
...
const LeadSetupPanel = ({ data, language, busy, mutate }: { data: WorkspaceData; language: Language; busy: boolean; mutate: Mutate }) => {
  ...
  const policySaved = useArrangementFormDraft(policyId)
  const childSaved = useArrangementFormDraft(photoUrl)
...
const EventDayPanel = ({ data, language, busy, mutate, onOccurrence }: { data: WorkspaceData; language: Language; busy: boolean; mutate: Mutate; onOccurrence: (id: string) => void }) => {
  ...
  return <AppSectionCard summary={language === 'zh' ? `显示 ${children.length} / ${data.children.length} 项儿童记录` : `Showing ${children.length} / ${data.children.length} child records`} title={language === 'zh' ? '场次签到与交接' : 'Occurrence check-in & collection'} subtitle={language === 'zh' ? '快速查找儿童，核对同意与授权接领状态，再执行签到或签出。' : 'Find a child quickly, verify consent and collection authority, then check in or check out.'} action={<AppBadge variant="info">{children.length}/{data.children.length}</AppBadge>}>
