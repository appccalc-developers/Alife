...
export const EventTeamPanel = ({ eventId, groupId, language, item, onBusyChange, onSaved, setupFlow }: EventSurfaceProps) => {
  ...
  useArrangementDraft(Boolean(inviteeId || taskTitleEn || taskTitleZh || taskDue || assigneeId || requiresApproval || isRestricted))
  ...
  const createTask = (event: FormEvent) => {
    ...
    void mutate(async () => {
      await eventOperationsService.createTask(eventId, { title: { en: taskTitleEn.trim(), zh: taskTitleZh.trim() }, description: { en: '', zh: '' }, assignedMemberId: assigneeId || null, dueUtc: taskDue ? fromLocalInput(taskDue) : null, isRequired: true, requiresApproval, isRestricted, reviewerMemberId: reviewerId || null, stage: taskStage, eventOccurrenceId: taskOccurrence || null })
      setTaskTitleEn(''); setTaskTitleZh(''); setTaskDue(''); setAssigneeId(''); setRequiresApproval(false); setIsRestricted(false); setReviewerId('')
    })
  }
  ...
  const groupedTasks = groupEventTasks((data?.tasks ?? []).filter(task => (filterStage === 'all' || (task.stage || 'preparation') === filterStage) && (!taskParams.get('occurrenceId') || task.eventOccurrenceId === taskParams.get('occurrenceId'))))
