...
const surfaceStatusSummary = (state: AsyncState, language: 'en' | 'zh') => {
  const labels: Partial<Record<AsyncState, string>> = language === 'zh'
    ? { loading: '正在读取…', error: '读取失败，请展开重试', 'permission-denied': '没有查看权限', conflict: '资料已变更，请展开重新读取' }
    : { loading: 'Loading…', error: 'Loading failed; expand to retry', 'permission-denied': 'Access required', conflict: 'Data changed; expand to reload' }
  return labels[state]
}
...
export const EventTeamPanel = ({ eventId, groupId, language, item, onBusyChange, onSaved, setupFlow, moduleSelections, planRevision, canManage }: EventSurfaceProps) => {
  ...
  const [taskDescription, setTaskDescription] = useState({ en: '', zh: '' })
  const [taskPage, setTaskPage] = useState(1), [taskSort, setTaskSort] = useState('due'), [filterStatus, setFilterStatus] = useState('all')
  ...
  useArrangementDraft(Boolean(inviteeId || taskTitleEn || taskTitleZh || taskDescription.en || taskDescription.zh || taskDue || assigneeId || requiresApproval || isRestricted))
  const load = useCallback(async () => {
    ...
    } catch (reason) { if (version === teamRequest.current) { setState(resolveState(reason)); setError(normalizeApiError(reason).message) } }
  }, [eventId, groupId, currentMemberId, planRevision])

  useEffect(() => { setData(null); setCandidates([]); void load(); return () => { teamRequest.current++ } }, [load])
  useEffect(() => { onBusyChange?.(busy || aiBusy) }, [busy, aiBusy, onBusyChange])
  ...
  const createTask = (event: FormEvent) => {
    ...
    if (busy || aiBusy || !canManage || !taskTitleEn.trim() || !taskTitleZh.trim()) return
    void mutate(async () => {
      await eventOperationsService.createTask(eventId, { title: { en: taskTitleEn.trim(), zh: taskTitleZh.trim() }, description: taskDescription, activityId: taskActivity || null, requireAcceptance: true, assignedMemberId: assigneeId || null, dueUtc: taskDue ? fromLocalInput(taskDue) : null, isRequired: true, requiresApproval, isRestricted, reviewerMemberId: reviewerId || null, stage: taskStage, eventOccurrenceId: taskOccurrence || null })
      setTaskActivity(''); setTaskTitleEn(''); setTaskTitleZh(''); setTaskDescription({ en: '', zh: '' }); setTaskDue(''); setAssigneeId(''); setRequiresApproval(false); setIsRestricted(false); setReviewerId('')
    })
  }
  ...
  const taskRows = (data?.tasks ?? []).filter(task => (filterStage === 'all' || (task.stage || 'preparation') === filterStage) && (filterStatus === 'all' || task.status === filterStatus) && (!taskParams.get('occurrenceId') || task.eventOccurrenceId === taskParams.get('occurrenceId')))
    .sort((a, b) => taskSort === 'title' ? localize(a.title, language).localeCompare(localize(b.title, language), language) : (a.dueUtc || '9999').localeCompare(b.dueUtc || '9999'))
  const pages = Math.max(1, Math.ceil(taskRows.length / 10)), page = Math.min(taskPage, pages)
  const groupedTasks = groupEventTasks(taskRows.slice((page - 1) * 10, page * 10))
  const eligibleCandidates = candidates.filter(candidate => candidate.id === currentMemberId || data?.members.some(member => member.memberId === candidate.id && member.status === 'accepted') || data?.roles.some(role => role.memberId === candidate.id && role.status === 'accepted'))
    ...
    <div className="space-y-4">
      ...
      <AppSectionCard summary={surfaceStatusSummary(state, language) ?? (language === 'zh' ? `${data?.members.length || 0} 名协作成员` : `${data?.members.length || 0} collaborators`)} title={language === 'zh' ? '协作成员' : 'Collaborators'} subtitle={language === 'zh' ? '邀请成员协助筹备，成员需在 App 内接受或拒绝。' : 'Invite members to help prepare. Each member accepts or declines in the app.'} action={<AppBadge variant={item.readiness === 'ready' ? 'success' : 'warning'}>{item.readiness}</AppBadge>}>
        ...
        {data ? <div className="space-y-6">
          ...
          <section aria-labelledby="event-team-heading">
            ...
            {data.canManage ? <form className="mt-4 flex flex-col gap-2 tablet:flex-row" onSubmit={(event) => { event.preventDefault(); if (inviteeId) void mutate(() => eventOperationsService.inviteTeamMember(eventId, inviteeId).then(value => { setInviteeId(''); return value })) }}><label className="sr-only" htmlFor="event-team-invitee">{language === 'zh' ? '邀請成員' : 'Invite member'}</label><select id="event-team-invitee" value={inviteeId} onChange={(event) => setInviteeId(event.target.value)} className={fieldClass}><option value="">{language === 'zh' ? '選擇小組成員' : 'Select group member'}</option>{candidates.filter((candidate) => !data.members.some((member) => member.memberId === candidate.id && !member.endedUtc && ['invited', 'accepted'].includes(member.status))).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.displayName || candidate.id}</option>)}</select><AppActionButton type="submit" variant="primary" disabled={busy || !inviteeId}>{language === 'zh' ? '發出邀請' : 'Send invitation'}</AppActionButton></form> : null}
          </section>
        </div> : null}
      </AppSectionCard>

      {data ? <AppSectionCard summary={surfaceStatusSummary(state, language) ?? (language === 'zh' ? `${data.tasks.length} 项任务 · ${data.tasks.filter(task => task.status === 'done').length} 项完成` : `${data.tasks.length} tasks · ${data.tasks.filter(task => task.status === 'done').length} done`)} title={language === 'zh' ? '自定义任务与准备情况' : 'Custom tasks and preparation'} subtitle={language === 'zh' ? '安排模块之外的具体任务，由负责人接受并填写准备情况。' : 'Delegate work beyond the enabled modules. Assignees accept and record their preparation.'}>
        {data.canManage ? <DetailsWorkspace zh={language === 'zh'} active readOnly={busy || !canManage}
          labels={{ workspace: language === 'zh' ? '任务与交接工作区' : 'Tasks and handoffs workspace', form: language === 'zh' ? '任务表单' : 'Task form', assistant: language === 'zh' ? 'AI 任务资料助手' : 'AI task assistant' }}
          assistant={active => <EventFormAssistant key={`${currentMemberId}:${eventId}`} eventId={eventId} scope="tasks" zh={language === 'zh'} active={active} onBusy={setAiBusy}
            form={{ title: { en: taskTitleEn, zh: taskTitleZh }, dueLocal: taskDue, stage: taskStage, requiresApproval, isRestricted }}
            contextSignature={JSON.stringify([assigneeId, reviewerId, taskOccurrence, taskActivity, taskDescription])}
            onAdopt={(form, fields) => {
              if (fields.includes('title')) { const title = form.title as Bilingual; setTaskTitleEn(title.en); setTaskTitleZh(title.zh) }
              if (fields.includes('dueLocal')) setTaskDue(form.dueLocal as string)
              if (fields.includes('stage')) setTaskStage(form.stage as string)
              if (fields.includes('requiresApproval')) setRequiresApproval(form.requiresApproval as boolean)
              if (fields.includes('isRestricted')) setIsRestricted(form.isRestricted as boolean)
            }} />}
          form={<form className="mb-5" onSubmit={createTask}><fieldset disabled={busy || !canManage} className="grid gap-3 tablet:grid-cols-2">
            <label className={labelClass}>{language === 'zh' ? '关联活动项目（可选）' : 'Linked activity (optional)'}<select className={fieldClass} value={taskActivity} onChange={e => setTaskActivity(e.target.value)}><option value="">—</option>{activityPlan?.activities.map(activity => <option key={activity.id} value={activity.id}>{localize(activity.name, language)}</option>)}</select></label>
            <div data-detail-field="title" className="min-w-0 tablet:col-span-2"><BilingualField primaryLanguage={language} label={language === 'zh' ? '任务标题' : 'Task title'} required maxLength={300} value={{ en: taskTitleEn, zh: taskTitleZh }} onChange={value => { setTaskTitleEn(value.en); setTaskTitleZh(value.zh) }} /></div>
            <div className="min-w-0 tablet:col-span-2"><BilingualField primaryLanguage={language} label={language === 'zh' ? '任务描述' : 'Task description'} multiline maxLength={2000} value={taskDescription} onChange={setTaskDescription} /></div>
            <label className={labelClass}>{language === 'zh' ? '负责人' : 'Assignee'}<select required className={fieldClass} value={assigneeId} onChange={e => setAssigneeId(e.target.value)}><option value="">{language === 'zh' ? '未指派' : 'Unassigned'}</option>{eligibleCandidates.map(member => <option key={member.id} value={member.id}>{member.displayName || member.id}</option>)}</select></label>
            <label data-detail-field="dueLocal" className={labelClass}>{language === 'zh' ? '期限' : 'Due'}<input type="datetime-local" className={fieldClass} value={taskDue} onChange={e => setTaskDue(e.target.value)} /></label>
            <label data-detail-field="stage" className={labelClass}>{language === 'zh' ? '所属阶段' : 'Stage'}<select className={fieldClass} value={taskStage} onChange={e => setTaskStage(e.target.value)}>{workStages.map(stage => <option key={stage} value={stage}>{workStageText(stage, language === 'zh')}</option>)}</select></label>
            <label className={labelClass}>{language === 'zh' ? '关联场次' : 'Occurrence'}<select className={fieldClass} value={taskOccurrence} onChange={e => setTaskOccurrence(e.target.value)}><option value="">{language === 'zh' ? '整个活动' : 'Whole event'}</option>{taskOccurrences.map(occurrence => <option key={occurrence.id} value={occurrence.id}>{new Date(occurrence.startUtc).toLocaleString(language)}</option>)}</select></label>
            <label data-detail-field="requiresApproval" className="flex min-h-11 items-center gap-2 text-sm font-bold text-[#40554e]"><input type="checkbox" checked={requiresApproval} onChange={e => setRequiresApproval(e.target.checked)} />{language === 'zh' ? '需要批准' : 'Requires approval'}</label>
            {requiresApproval ? <label className={labelClass}>{language === 'zh' ? '指定审核人' : 'Assigned reviewer'}<select className={fieldClass} value={reviewerId} onChange={e => setReviewerId(e.target.value)}><option value="">{language === 'zh' ? '默认负责人（不能自审）' : 'Default: owner (no self-review)'}</option>{candidates.filter(candidate => candidate.id !== assigneeId && (candidate.id === currentMemberId || data.members.some(member => member.memberId === candidate.id && member.status === 'accepted') || data.roles.some(role => role.memberId === candidate.id && role.status === 'accepted'))).map(candidate => <option key={candidate.id} value={candidate.id}>{candidate.displayName || candidate.id}</option>)}</select></label> : null}
            <label data-detail-field="isRestricted" className="flex min-h-11 items-center gap-2 text-sm font-bold text-[#40554e]"><input type="checkbox" checked={isRestricted} onChange={e => setIsRestricted(e.target.checked)} />{language === 'zh' ? '限制任务' : 'Restricted task'}</label>
            <AppActionButton className="self-end" type="submit" variant="primary" disabled={busy || aiBusy || !canManage}>{language === 'zh' ? '新增任务' : 'Add task'}</AppActionButton>
          </fieldset></form>} /> : null}
        <label className="mb-3 grid gap-2 text-sm">{language === 'zh' ? '按阶段查看任务' : 'Tasks by stage'}<select className={fieldClass} value={filterStage} onChange={e => { setFilterStage(e.target.value); setTaskPage(1) }}><option value="all">{language === 'zh' ? '所有阶段' : 'All stages'}</option>{workStages.map(stage => <option key={stage} value={stage}>{workStageText(stage, language === 'zh')}</option>)}</select></label><div className="mb-3 grid gap-3 tablet:grid-cols-2"><label className={labelClass}>{language === 'zh' ? '任务状态' : 'Task status'}<select className={fieldClass} value={filterStatus} onChange={e => { setFilterStatus(e.target.value); setTaskPage(1) }}><option value="all">{language === 'zh' ? '所有状态' : 'All statuses'}</option>{taskColumns.map(column => <option key={column.status} value={column.status}>{column[language]}</option>)}</select></label><label className={labelClass}>{language === 'zh' ? '排序' : 'Sort'}<select className={fieldClass} value={taskSort} onChange={e => { setTaskSort(e.target.value); setTaskPage(1) }}><option value="due">{language === 'zh' ? '按期限' : 'Due date'}</option><option value="title">{language === 'zh' ? '按标题' : 'Title'}</option></select></label></div>{taskRows.length ? <div className="grid gap-3 desktop:grid-cols-2">{taskColumns.filter(column => groupedTasks[column.status].length).map((column) => <section key={column.status} className="rounded-2xl bg-[#eef4ef] p-3"><h3 className="mb-3 text-sm font-black text-[#18332d]">{language === 'zh' ? column.zh : column.en} <span className="text-[#66766f]">{groupedTasks[column.status].length}</span></h3><div className="space-y-3">{groupedTasks[column.status].map((task) => <TaskCard eventId={eventId} key={task.id} task={task} tasks={data.tasks} canManage={data.canManage} language={language} busy={busy} onStatus={(status) => void mutate(() => eventOperationsService.updateTask(eventId, task, status))} onDependency={(dependsOnTaskId) => void mutate(() => eventOperationsService.addTaskDependency(eventId, task.id, dependsOnTaskId))} onBlock={(reason) => void mutate(() => eventOperationsService.addTaskBlocker(eventId, task.id, reason))} onResolve={(blockerId, resolution) => void mutate(() => eventOperationsService.resolveTaskBlocker(eventId, task.id, blockerId, resolution))} />)}</div></section>)}</div> : <AppEmptyState title={language === 'zh' ? '尚無任務' : 'No tasks yet'} description={language === 'zh' ? '新增必要任務後，準備度會反映阻塞與逾期狀態。' : 'Required tasks feed blockers and overdue status into readiness.'} />}
        <div className="mt-4 flex flex-wrap items-center gap-3"><AppActionButton disabled={page === 1} onClick={() => setTaskPage(page - 1)}>{language === 'zh' ? '上一页' : 'Previous'}</AppActionButton><span className="text-sm">{page} / {pages}</span><AppActionButton disabled={page === pages} onClick={() => setTaskPage(page + 1)}>{language === 'zh' ? '下一页' : 'Next'}</AppActionButton></div>
      </AppSectionCard> : null}
    </div>
  )
}

const TaskCard = ({ eventId, task, tasks, canManage, language, busy, onStatus, onDependency, onBlock, onResolve }: { eventId: string; task: EventTask; tasks: EventTask[]; canManage: boolean; language: 'en' | 'zh'; busy: boolean; onStatus: (status: EventTaskStatus) => void; onDependency: (dependsOnTaskId: string) => void; onBlock: (reason: string) => void; onResolve: (blockerId: string, resolution: string) => void }) => {
  const currentMemberId = useAuthStore().me?.id
  ...
  const availableStatuses = (canManage ? ['todo', 'inProgress', 'blocked', 'done', 'cancelled'] : ['todo', 'inProgress', 'blocked', 'done']).filter(status => status !== 'done' || !task.requiresApproval || task.status === 'done') as EventTaskStatus[]
  return <article className="rounded-xl border border-[#2f4b42]/10 bg-white p-3 shadow-sm"><div className="flex items-start justify-between gap-2"><Link className="font-bold text-[#176b5a] underline underline-offset-4" to={`/events/${eventId}/tasks/${task.id}`}>{localize(task.title, language)}</Link>{task.isRequired ? <AppBadge variant="warning">{language === 'zh' ? '必要' : 'Required'}</AppBadge> : null}</div>{task.assignmentStatus ? <p className="mt-2 text-sm">{roleStatusLabel(task.assignmentStatus, language === 'zh')}</p> : null}{task.preparationUpdatedUtc ? <p className="mt-1 text-xs text-[#66766f]">{language === 'zh' ? '已填写准备情况' : 'Preparation recorded'}</p> : null}{task.dueUtc ? <time className="mt-2 block text-xs text-[#66766f]">{new Date(task.dueUtc).toLocaleString()}</time> : null}{task.requiresApproval ? <p className="mt-1 text-xs font-bold text-amber-800">{language === 'zh' ? '需完成批准' : 'Approval required'}</p> : null}<select className={`${fieldClass} mt-3`} value={task.status} aria-label={language === 'zh' ? '任务进度' : 'Task progress'} disabled={busy || (!canManage && task.assignedMemberId !== currentMemberId) || (task.assignmentStatus !== undefined && task.assignmentStatus !== 'accepted') || Boolean(task.sourceType) || task.approvalStatus === 'pendingReview'} onChange={(event) => onStatus(event.target.value as EventTaskStatus)}>{availableStatuses.map((status) => <option key={status} value={status}>{({ todo: language === 'zh' ? '待处理' : 'To do', inProgress: language === 'zh' ? '进行中' : 'In progress', blocked: language === 'zh' ? '受阻' : 'Blocked', done: language === 'zh' ? '已完成' : 'Done', cancelled: language === 'zh' ? '已取消' : 'Cancelled' })[status]}</option>)}</select>{task.dependencies.length ? <ul className="mt-2 space-y-1 text-xs text-[#66766f]">{task.dependencies.map((dependency) => <li key={dependency.id}>{language === 'zh' ? '依賴：' : 'Depends on: '}{localize(tasks.find((candidate) => candidate.id === dependency.dependsOnEventTaskId)?.title ?? { en: dependency.dependsOnEventTaskId, zh: dependency.dependsOnEventTaskId }, language)}</li>)}</ul> : null}{canManage && dependencyCandidates.length ? <div className="mt-2 flex gap-2"><select aria-label={language === 'zh' ? '依賴任務' : 'Dependency task'} className={fieldClass} value={dependencyId} onChange={(event) => setDependencyId(event.target.value)}><option value="">{language === 'zh' ? '新增依賴' : 'Add dependency'}</option>{dependencyCandidates.map((candidate) => <option key={candidate.id} value={candidate.id}>{localize(candidate.title, language)}</option>)}</select><AppActionButton size="sm" disabled={busy || !dependencyId} onClick={() => { onDependency(dependencyId); setDependencyId('') }}>{language === 'zh' ? '加入' : 'Add'}</AppActionButton></div> : null}{task.blockers.filter((blocker) => !blocker.resolvedUtc).map((blocker) => { const resolution = resolutions[blocker.id] ?? ''; return <div key={blocker.id} className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2"><p className="text-xs text-amber-950">{blocker.reason}</p><div className="mt-2 flex gap-2"><input className={fieldClass} value={resolution} onChange={(event) => setResolutions((current) => ({ ...current, [blocker.id]: event.target.value }))} placeholder={language === 'zh' ? '解除說明' : 'Resolution'} /><AppActionButton size="sm" disabled={busy || !resolution.trim()} onClick={() => { onResolve(blocker.id, resolution.trim()); setResolutions((current) => ({ ...current, [blocker.id]: '' })) }}>{language === 'zh' ? '解除' : 'Resolve'}</AppActionButton></div></div> })}{task.status !== 'done' ? <div className="mt-2 flex gap-2"><input className={fieldClass} value={reason} onChange={(event) => setReason(event.target.value)} placeholder={language === 'zh' ? '阻塞原因' : 'Blocker reason'} /><AppActionButton size="sm" disabled={busy || !reason.trim()} onClick={() => { onBlock(reason.trim()); setReason('') }}>{language === 'zh' ? '阻塞' : 'Block'}</AppActionButton></div> : null}</article>
}
...
function useRosterStore(eventId: string, groupId: string) {
  ...
  useEffect(() => { let live = true; void eventOperationsService.listOccurrences(eventId).then(list => { if (live) { setOccurrences(list); setOccurrenceId(list.find(x => x.id === new URLSearchParams(window.location.search).get('occurrenceId'))?.id || list[0]?.id || '') } }).catch(reason => { if (live) { setState(resolveState(reason)); setError(normalizeApiError(reason).message) } }); return () => { live = false } }, [eventId])
  const load = useCallback(async () => {
