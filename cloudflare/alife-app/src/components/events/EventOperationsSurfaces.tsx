...
export const EventTeamPanel = ({ eventId, groupId, language, item, eventBasePath, onBusyChange, onSaved, setupFlow }: EventSurfaceProps) => {
  ...
  return (
    <div className="space-y-4">
      <AppSectionCard title={localize(item.label, language)} subtitle={language === 'zh' ? '这里只保留跨模块的协调任务；岗位人选及轮班在对应模块中安排。' : 'Keep cross-module coordination tasks here. Role candidates and shifts belong to their modules.'} action={<AppBadge variant={item.readiness === 'ready' ? 'success' : 'warning'}>{item.readiness}</AppBadge>}>
        ...
        {data ? <div className="space-y-6">
          ...
          <details><summary className="cursor-pointer font-semibold text-[#176b5a]">{language === 'zh' ? '协作成员与历史分工' : 'Collaborators and historical assignments'}</summary><section aria-labelledby="event-team-heading">
            <div className="flex flex-wrap items-center justify-between gap-3"><h3 id="event-team-heading" className="font-black text-[#18332d]">{setupFlow ? (language === 'zh' ? '团队成员' : 'Team members') : (language === 'zh' ? '團隊與角色' : 'Team & roles')}</h3><Link className="text-sm font-bold text-[#176b5a] hover:underline" to={`${eventBasePath}?section=workflow`}>{language === 'zh' ? '流程與產出物' : 'Workflow & outputs'}</Link></div>
...
const RosterWorkspaceContent = ({ eventId, language, item, onBusyChange, onSaved, rosterModule, store }: EventSurfaceProps & { rosterModule?: string; store: ReturnType<typeof useRosterStore> }) => {
  ...
  const [expanded, setExpanded] = useState(!rosterModule)
  useEffect(() => { if (data?.slots.length) setExpanded(true) }, [data?.slots.length])
  ...
  return rosterModule ? <details open={expanded} onToggle={event => setExpanded(event.currentTarget.open)} className="mt-3 rounded-xl border border-[#176b5a]/15 p-3"><summary className="cursor-pointer font-semibold">{language === 'zh' ? '岗位轮班' : 'Role shifts'} · {data?.slots.length || 0}</summary><div className="mt-3">{content}</div></details> : content
}
