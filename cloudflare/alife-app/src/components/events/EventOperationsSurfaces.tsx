...
export const EventTeamPanel = ({ eventId, groupId, language, item, onBusyChange, onSaved, setupFlow, moduleSelections, planRevision }: EventSurfaceProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {data ? <AppSectionCard summary={surfaceStatusSummary(state, language) ?? (language === 'zh' ? `${data.tasks.length} 项任务 · ${data.tasks.filter(task => task.status === 'done').length} 项完成` : `${data.tasks.length} tasks · ${data.tasks.filter(task => task.status === 'done').length} done`)} title={language === 'zh' ? '自定义任务与准备情况' : 'Custom tasks and preparation'} subtitle={language === 'zh' ? '安排模块之外的具体任务，由负责人接受并填写准备情况。' : 'Delegate work beyond the enabled modules. Assignees accept and record their preparation.'}>
        {data.canManage ? <form className="mb-5 grid gap-3 tablet:grid-cols-2 desktop:grid-cols-4" onSubmit={createTask}><label className="text-sm">{language==='zh'?'关联活动项目（可选）':'Linked activity (optional)'}<select className={fieldClass} value={taskActivity} onChange={e=>setTaskActivity(e.target.value)}><option value="">—</option>{activityPlan?.activities.map(a=><option key={a.id} value={a.id}>{localize(a.name,language)}</option>)}</select></label><BilingualField primaryLanguage={language} label={language === 'zh' ? '任务标题' : 'Task title'} required maxLength={300} value={{ en: taskTitleEn, zh: taskTitleZh }} onChange={value => { setTaskTitleEn(value.en); setTaskTitleZh(value.zh) }} /><BilingualField primaryLanguage={language} label={language === 'zh' ? '任务描述' : 'Task description'} multiline maxLength={2000} value={taskDescription} onChange={setTaskDescription} /><label className={labelClass}>{language === 'zh' ? '負責人' : 'Assignee'}<select required className={fieldClass} value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)}><option value="">{language === 'zh' ? '未指派' : 'Unassigned'}</option>{eligibleCandidates.map(member => <option key={member.id} value={member.id}>{member.displayName || member.id}</option>)}</select></label><label className={labelClass}>{language === 'zh' ? '期限' : 'Due'}<input type="datetime-local" className={fieldClass} value={taskDue} onChange={(event) => setTaskDue(event.target.value)} /></label><label className={labelClass}>{language === 'zh' ? '所属阶段' : 'Stage'}<select className={fieldClass} value={taskStage} onChange={e => setTaskStage(e.target.value)}>{workStages.map(stage => <option key={stage} value={stage}>{workStageText(stage, language === 'zh')}</option>)}</select></label><label className={labelClass}>{language === 'zh' ? '关联场次' : 'Occurrence'}<select className={fieldClass} value={taskOccurrence} onChange={e => setTaskOccurrence(e.target.value)}><option value="">{language === 'zh' ? '整个活动' : 'Whole event'}</option>{taskOccurrences.map(o => <option key={o.id} value={o.id}>{new Date(o.startUtc).toLocaleString(language)}</option>)}</select></label><label className="flex min-h-11 items-center gap-2 text-sm font-bold text-[#40554e]"><input type="checkbox" checked={requiresApproval} onChange={(event) => setRequiresApproval(event.target.checked)} />{language === 'zh' ? '需要批准' : 'Requires approval'}</label>{requiresApproval ? <label className={labelClass}>{language === 'zh' ? '指定审核人' : 'Assigned reviewer'}<select className={fieldClass} value={reviewerId} onChange={e => setReviewerId(e.target.value)}><option value="">{language === 'zh' ? '默认负责人（不能自审）' : 'Default: owner (no self-review)'}</option>{candidates.filter(m => m.id !== assigneeId && (m.id === currentMemberId || data.members.some(t => t.memberId === m.id && t.status === 'accepted') || data.roles.some(r => r.memberId === m.id && r.status === 'accepted'))).map(m => <option key={m.id} value={m.id}>{m.displayName || m.id}</option>)}</select></label> : null}<label className="flex min-h-11 items-center gap-2 text-sm font-bold text-[#40554e]"><input type="checkbox" checked={isRestricted} onChange={(event) => setIsRestricted(event.target.checked)} />{language === 'zh' ? '限制任務' : 'Restricted task'}</label><AppActionButton className="self-end" type="submit" variant="primary" disabled={busy}>{language === 'zh' ? '新增任務' : 'Add task'}</AppActionButton></form> : null}
