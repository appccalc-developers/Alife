...
export const EventTeamPanel = ({ eventId, groupId, language, item, onBusyChange, onSaved, setupFlow }: EventSurfaceProps) => {
  ...
  const createTask = (event: FormEvent) => {
    ...
    void mutate(async () => {
      await eventOperationsService.createTask(eventId, { title: { en: taskTitleEn.trim(), zh: taskTitleZh.trim() }, description: { en: '', zh: '' }, assignedMemberId: assigneeId || null, dueUtc: taskDue ? fromLocalInput(taskDue) : null, isRequired: true, requiresApproval, isRestricted, reviewerMemberId: reviewerId || null })
