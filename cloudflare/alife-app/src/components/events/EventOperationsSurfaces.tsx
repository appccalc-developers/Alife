...
export const EventTeamPanel = ({ eventId, groupId, language, item, onBusyChange, onSaved, setupFlow, moduleSelections, planRevision }: EventSurfaceProps) => {
  ...
  const createTask = (event: FormEvent) => {
    ...
    void mutate(async () => {
      await eventOperationsService.createTask(eventId, { title: { en: taskTitleEn.trim(), zh: taskTitleZh.trim() }, description: taskDescription, requireAcceptance: true, assignedMemberId: assigneeId || null, dueUtc: taskDue ? fromLocalInput(taskDue) : null, isRequired: true, requiresApproval, isRestricted, reviewerMemberId: reviewerId || null, stage: taskStage, eventOccurrenceId: taskOccurrence || null })
      setTaskTitleEn(''); setTaskTitleZh(''); setTaskDescription({ en: '', zh: '' }); setTaskDue(''); setAssigneeId(''); setRequiresApproval(false); setIsRestricted(false); setReviewerId('')
    })
  }
