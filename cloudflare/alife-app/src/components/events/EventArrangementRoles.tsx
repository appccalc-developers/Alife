  ...
  return <div className="space-y-3" data-arrangement-roles>
    ...
    {roles.map(role => {
      ...
      return <section key={role.requirementKey} aria-label={arrangementRoleLabel(role.roleCode, zh)} className="min-w-0 rounded-xl border border-[#176b5a]/15 bg-white p-3">
        ...
        {assignments.map(assignment => <div key={assignment.id} className="mt-2 flex flex-wrap items-center gap-2 text-sm"><span className="break-all">{state?.candidates.find(x => x.id === assignment.memberId)?.displayName || state?.data?.members.find(x => x.memberId === assignment.memberId)?.displayName || assignment.memberId}</span><span className="text-[#66766f]">{roleStatusLabel(assignment.status, zh)}</span>{!summary && !readOnly && assignment.memberId === currentMemberId && assignment.status === 'invited' && eventId ? <><AppActionButton size="sm" disabled={busy} onClick={() => void mutate(role, () => eventOperationsService.respondToRoleInvitation(eventId, assignment.id, true))}>{zh ? '接受角色' : 'Accept role'}</AppActionButton><AppActionButton size="sm" disabled={busy} onClick={() => void mutate(role, () => eventOperationsService.respondToRoleInvitation(eventId, assignment.id, false))}>{zh ? '拒绝' : 'Decline'}</AppActionButton></> : null}</div>)}
