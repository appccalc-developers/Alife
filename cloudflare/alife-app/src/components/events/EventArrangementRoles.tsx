  ...
  return <div className={compact ? "space-y-2" : "space-y-3"} data-arrangement-roles>
    ...
    {roles.map(role => {
      ...
      return <section key={role.requirementKey} aria-label={arrangementRoleLabel(role.roleCode, zh)} className={compact ? "min-w-0 border-t border-violet-200/60 py-2 first:border-t-0" : "min-w-0 rounded-xl border border-[#176b5a]/15 bg-white p-3"}>
        ...
        {role.roleCode !== 'event.accountableOwner' && !summary && !readOnly && state?.data?.canManage && active && eventId ? <div className="mt-3 flex min-w-0 flex-wrap gap-2"><label className="min-w-0 flex-1 text-xs"><span className={compact ? 'sr-only' : undefined}>{compact ? `${arrangementRoleLabel(role.roleCode, zh)} · ${zh ? '选择成员' : 'Select member'}` : (zh ? '邀请成员承担此角色' : 'Invite a member for this role')}</span><select className="mt-1 min-h-11 w-full min-w-0 rounded-xl border bg-white px-2 text-sm" value={invitees[role.requirementKey] || ''} onChange={event => setInvitees(previous => ({ ...previous, [role.requirementKey]: event.target.value }))}><option value="">{zh ? '选择成员' : 'Select member'}</option>{state.candidates.filter(candidate => !assignments.some(x => x.memberId === candidate.id && ['invited', 'accepted'].includes(x.status))).map(candidate => <option key={candidate.id} value={candidate.id}>{candidate.displayName || candidate.id}</option>)}</select></label><AppActionButton className="self-end" disabled={busy || !invitees[role.requirementKey]} onClick={() => void mutate(role, () => eventOperationsService.createRoleInvitation(eventId, role.requirementKey, invitees[role.requirementKey]))}>{zh ? '邀请' : 'Invite'}</AppActionButton></div> : null}
