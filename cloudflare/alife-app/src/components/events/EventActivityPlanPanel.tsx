...
import { activityPlanAssistantForm, applyActivityPlanAssistantForm } from '../../utils/eventFormAssistant'
...
export function ActivityPlanEditor({ data, onChange, zh, disabled = false, occurrences = [] }: { data: ActivityPlanData; onChange: (data: ActivityPlanData) => void; zh: boolean; disabled?: boolean; occurrences?: {id:string; startUtc:string}[] }) {
  ...
  return <RamEditingLanguage.Provider value={zh}><fieldset disabled={disabled} className="space-y-3" data-activity-plan-editor>
    <label data-detail-field="participantCount" className="block text-sm">{zh ? '预计人数' : 'Expected participants'}<input className={ramInput} type="number" min={1} max={1000000} value={data.participantCount ?? ''} onChange={e=>update({participantCount:e.target.value ? Number(e.target.value) : null})}/></label>
    <div className="flex flex-wrap gap-4">{(['isOuting','isOvernight','isHighRisk'] as const).map((key,i)=><label data-detail-field={key} key={key} className="flex min-h-11 items-center gap-2 text-sm"><input type="checkbox" checked={data[key]} onChange={e=>update({[key]:e.target.checked})}/>{(zh?['室外／场外','过夜','已知高风险活动']:['Outdoor / off-site','Overnight','Known high-risk activity'])[i]}</label>)}</div>
    <div data-detail-field="activities" className="space-y-3">{data.activities.map(a=><details key={a.id} className="rounded-xl border border-violet-200 bg-violet-50/40 px-3" data-planned-activity><summary className="min-h-11 cursor-pointer py-3 font-semibold">{text(a.name,zh)|| (zh?'未命名活动':'Unnamed activity')} <span className="text-xs font-normal">{text(ramActivityLabels[a.type],zh)}</span></summary><div className="space-y-3 pb-3">
    ...
    </div><AppActionButton disabled={data.activities.length>=50} onClick={()=>update({activities:[...data.activities,{id:crypto.randomUUID(),type:'generic',name:{en:'',zh:''},conditions:{en:'',zh:''}}]})}>{zh?'添加活动项目':'Add activity'}</AppActionButton>
...
export default function EventActivityPlanPanel({eventId,groupId,zh,eventContext,onSaved,onPlanChange,onBusy}:{eventId:string;groupId:string;zh:boolean;eventContext?:FormAssistantContext;onSaved?:()=>Promise<void>;onPlanChange?:(plan:ActivityPlanData)=>void;onBusy?:(busy:boolean)=>void}) {
  ...
  return <EventToolSection title={zh?'活动项目与条件':'Activities and conditions'} summary={view?`${draft.activities.length} ${zh?'个活动项目':'activities'}`:(zh?'正在读取':'Loading')}>
    ...
    {!view?<AppActionButton onClick={()=>setReload(x=>x+1)}>{zh?'重新读取':'Reload'}</AppActionButton>:<div className="space-y-3">
      ...
      <DetailsWorkspace zh active readOnly={!view.canEdit||busy} limitAssistantHeight labels={{ workspace: zh ? '活动项目与条件工作区' : 'Activities and conditions workspace', form: zh ? '活动条件表单' : 'Activity conditions form', assistant: zh ? 'AI 活动资料助手' : 'AI activity assistant' }}
        form={<ActivityPlanEditor data={draft} onChange={setDraft} zh={zh} disabled={!view.canEdit||busy} occurrences={occurrences}/>}
        assistant={active => <EventFormAssistant eventId={eventId} scope="activityPlan" form={activityPlanAssistantForm(draft)} context={eventContext} contextSignature={JSON.stringify([draft, eventContext])} zh={zh} active={active} initiallyConfirmed onBusy={onBusy}
          onAdopt={(form, fields) => setDraft(current => JSON.stringify(current) === JSON.stringify(draft) ? applyActivityPlanAssistantForm(current, form, fields) : current)} />} />
