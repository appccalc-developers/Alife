...
export default function EventRamWorkspace({ eventId, language, onDirtyChange, onBusyChange, onSaved, pendingEventChanges = false, showEventPlan = false }: { eventId: string; language: string; onDirtyChange?: (dirty: boolean) => void; onBusyChange?: (busy: boolean) => void; onSaved?: () => Promise<void>; pendingEventChanges?: boolean; showEventPlan?: boolean }) {
  ...
  return <div className="space-y-5" data-ram-workspace>
    ...
    <AppSectionCard summary={`${draft.answers.filter(a => a.answer.en.trim() || a.answer.zh.trim() || a.notApplicable).length}`} title={zh ? '适用必答题' : 'Required questions'} subtitle={zh ? '每个活动分别回答；不适用时必须填写原因。题目帮助无需 AI 也能查看。' : 'Answer for each activity separately. Not applicable requires a reason. Question guidance is available without AI.'}>
    ...
    <AppSectionCard summary={text(ramValidityLabels[ram?.validity || 'Draft'], zh)} title={zh ? '本人确认与 RAM 独立审核' : 'Personal confirmation and independent RAM review'}>
    ...
    <AppSectionCard summary={`${workspace.history.length}`} title={zh ? '版本与签署历史' : 'Version and signature history'}>
