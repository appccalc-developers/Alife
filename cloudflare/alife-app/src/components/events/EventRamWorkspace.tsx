...
export default function EventRamWorkspace({ eventId, language, onDirtyChange, onBusyChange, onSaved, beforeAction, pendingEventChanges = false, showEventPlan = false }: { eventId: string; language: string; onDirtyChange?: (dirty: boolean) => void; onBusyChange?: (busy: boolean) => void; onSaved?: () => Promise<void>; beforeAction?: () => Promise<void>; pendingEventChanges?: boolean; showEventPlan?: boolean }) {
  ...
  const history = [...workspace.history].sort((a,b)=>b.version-a.version)
  ...
  return <div className="space-y-5" data-ram-workspace>
    <RamSyncPanel eventId={eventId} zh={zh} disabled={dirty || busy || authoringBusy || pendingEventChanges} onVersionChange={etag => { if (etag !== (workspace.assessment?.eTag || 'new')) { if (dirty || busy || authoringBusy) setNewVersion(true); else void load().catch(e => setError(normalizeApiError(e).message)) } }} />
    ...
    <div data-ram-field="ram.onsite"><AppSectionCard summary={text(ramValidityLabels[ram?.validity || 'Draft'], zh)} title={zh ? '本人确认与 RAM 独立审核' : 'Personal confirmation and independent RAM review'}>
      ...
      <div className="overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr>{(zh?['版本','时间','等级','签署与审批','操作']:['Version','Date','Level','Signatures and review','Actions']).map(h=><th key={h} className="p-2">{h}</th>)}</tr></thead><tbody>{history.map(r=><tr key={r.id} className="border-t align-top"><td className="p-2">v{r.version}</td><td className="p-2">{new Date(r.createdUtc).toLocaleString(zh?'zh':'en')}</td><td className="p-2"><RamLevelBadge zh={zh} level={r.residualLevel}/></td><td className="p-2">{workspace.actions.filter(a=>a.revisionId===r.id).map(a=><p key={a.id}>{text(ramActionLabels[a.action],zh)||a.action} · {workspace.onsiteCandidates.find(p=>p.memberId===a.actorMemberId)?.name||a.actorMemberId}{a.reason?<span className="block">{a.reason}</span>:null}</p>)}</td><td className="p-2"><AppActionButton disabled={busy} onClick={()=>void run(async()=>setPrint(await ramService.version(eventId,r.id)))}>{zh?'查看／打印':'View / print'}</AppActionButton><AppActionButton disabled={busy} onClick={()=>void run(async()=>setComparison(await ramService.version(eventId,r.id)))}>{zh?'对照':'Compare'}</AppActionButton></td></tr>)}</tbody></table>{!history.length?<p>{zh?'暂无固定版本':'No saved versions'}</p>:null}</div>
    </AppSectionCard>
