...
export const EventVenueWorkspaceSurface = ({ eventId, groupId, item, language, onBusyChange, onSaved }: EventSurfaceProps) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <AppSectionCard summary={language === 'zh' ? `${data.venues.length} 个场地${nameZh || nameEn ? ` · 新增：${nameZh || nameEn}` : ''}` : `${data.venues.length} venues${nameEn || nameZh ? ` · New: ${nameEn || nameZh}` : ''}`} title={language === 'zh' ? '場地目錄' : 'Venue catalogue'} subtitle={language === 'zh' ? '容量是預訂檢查的權威值；有確認預訂時不能停用或降到所需容量以下。' : 'Capacity is authoritative for reservations. A venue with confirmed bookings cannot be deactivated or reduced below demand.'}>
        {data.canManageCatalogue ? <form className="mb-5 grid gap-3 tablet:grid-cols-2 desktop:grid-cols-5" onSubmit={createVenue}><label className={labelClass}>English<input className={fieldClass} value={nameEn} onChange={(event) => setNameEn(event.target.value)} required /></label><label className={labelClass}>中文<input className={fieldClass} value={nameZh} onChange={(event) => setNameZh(event.target.value)} required /></label><label className={labelClass}>{language === 'zh' ? '英文地址' : 'Address (English)'}<input className={fieldClass} value={addressEn} onChange={(event) => setAddressEn(event.target.value)} /></label><label className={labelClass}>{language === 'zh' ? '中文地址' : 'Address (Chinese)'}<input className={fieldClass} value={addressZh} onChange={(event) => setAddressZh(event.target.value)} /></label><label className={labelClass}>{language === 'zh' ? '容量' : 'Capacity'}<input className={fieldClass} type="number" min="1" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} required /></label><AppActionButton className="tablet:col-span-2 desktop:col-span-1" type="submit" variant="primary" disabled={mutationState === 'saving'}>{language === 'zh' ? '加入目錄' : 'Add venue'}</AppActionButton></form> : null}
        {data.venues.length ? <div className="grid gap-3 tablet:grid-cols-2">{data.venues.map((venue) => <VenueCard key={venue.id} venue={venue} groupId={groupId} language={language} busy={mutationState === 'saving'} editable={data.canManageCatalogue && venue.managingGroupId === groupId} onMutate={mutate} />)}</div> : <AppEmptyState title={language === 'zh' ? '尚無場地' : 'No venues yet'} description={language === 'zh' ? '加入第一個可預訂場地，並設定可安全接待的人數。' : 'Add the first reservable venue and its safe capacity.'} />}
      </AppSectionCard>

      <AppSectionCard summary={language === 'zh' ? `${localize(data.venues.find(venue => venue.id === venueId)?.name || { en: '', zh: '' }, language) || '未选择场地'} · ${requiredCapacity} 人` : `${localize(data.venues.find(venue => venue.id === venueId)?.name || { en: '', zh: '' }, language) || 'No venue selected'} · ${requiredCapacity} people`} title={language === 'zh' ? '新增預訂' : 'Reserve a venue'} subtitle={language === 'zh' ? '邊界相接不算衝突；任何實際重疊都會由伺服器拒絕並指出場地和時段。' : 'Touching boundaries do not conflict. Any actual overlap is rejected by the server with the venue and interval.'}>
...
const VenueCard = ({ venue, groupId, language, busy, editable, onMutate }: { venue: EventVenue; groupId: string; language: 'en' | 'zh'; busy: boolean; editable: boolean; onMutate: (action: () => Promise<EventVenue>, success: string) => Promise<unknown> }) => {
  ...
  return <article className={`rounded-2xl border border-[#2f4b42]/10 bg-[#fbfcf8] p-4 ${venue.isActive ? '' : 'opacity-65'}`}><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h3 className="break-words font-black text-[#18332d]">{localize(venue.name, language)}</h3>{address ? <p className="mt-1 flex items-start gap-1.5 text-sm text-[#66766f]"><MapPin className="mt-0.5 h-4 w-4 shrink-0" aria-hidden="true" />{address}</p> : null}</div><div className="flex flex-wrap items-center gap-2">{!editable ? <AppBadge variant="info">{language === 'zh' ? '教會共享' : 'Church shared'}</AppBadge> : null}<AppBadge variant={venue.isActive ? 'success' : 'neutral'}>{venue.isActive ? (language === 'zh' ? '可預訂' : 'Active') : (language === 'zh' ? '停用' : 'Inactive')}</AppBadge></div></div>{editable ? <div className="mt-4 grid gap-3 tablet:grid-cols-[1fr_auto_auto]"><label className={labelClass}>{language === 'zh' ? '容量' : 'Capacity'}<input className={fieldClass} type="number" min="1" value={capacity} onChange={(event) => setCapacity(Number(event.target.value))} /></label><label className="flex min-h-11 items-center gap-2 self-end text-sm font-bold text-[#40554e]"><input type="checkbox" checked={active} onChange={(event) => setActive(event.target.checked)} />{language === 'zh' ? '啟用' : 'Active'}</label><AppActionButton className="self-end" size="sm" disabled={busy || (capacity === venue.capacity && active === venue.isActive)} onClick={() => void onMutate(() => eventVenueService.updateVenue(groupId, venue.id, venue.eTag, { name: venue.name, address: venue.address, capacity, isActive: active }), language === 'zh' ? '場地已更新。' : 'Venue updated.')}>{language === 'zh' ? '儲存' : 'Save'}</AppActionButton></div> : <p className="mt-3 text-sm text-[#66766f]">{language === 'zh' ? '由教會管理者維護；本小組可查看容量並預訂。' : 'Maintained by church managers; this group can view capacity and reserve it.'}</p>}</article>
}
