...
const AppSectionCard = ({ title, subtitle, action, dense = false, children }: Props) => (
  <article className={`alife-panel rounded-2xl ${dense ? 'p-4 sm:p-5' : 'p-5 sm:p-6'}`}>
    {title || subtitle || action ? (
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-[#2f4b42]/10 pb-4">
        <div className="min-w-0">
          {title ? <h2 className="text-base font-black leading-tight tracking-[-0.01em] text-[#18332d] sm:text-lg">{title}</h2> : null}
