...
const reasons: Record<string, [string, string]> = {
  ...
  members: ['还有其他成员记录（包括邀请、申请和历史记录）。', 'Other membership records remain, including invitations, requests or history.'],
  ...
  relatedRecords: ['还有联系人、论坛、邀请或其他关联资料，需要先处理。', 'Contacts, forum content, invitations or other linked records need attention first.'],
}
...
export default function GroupDissolutionPanel({ groupId, groupName, language, beforeDissolve, onDissolved }: Props) {
  ...
  const dissolve = async () => {
    ...
    try {
      if (!await requestConfirmation({
        ...
        description: zh
          ? '此操作会永久删除这个空小组及你的组内成员关系，无法撤销。你的账号、其他小组身份和审计记录会保留。服务器会再次检查解散条件。'
          : 'This permanently deletes the empty group and your membership in it. It cannot be undone. Your account, other memberships and audit history remain. The server will check the requirements again.',
  ...
  return <section aria-label={zh ? '解散小组' : 'Dissolve group'} className="space-y-3 border-t border-rose-200 pt-5">
    ...
    <p className="text-sm text-[#66766f]">{zh
      ? '仅限只有组长一名成员，且没有下属事工组、页面、活动、相册或公告的空小组。草稿与已关闭内容也会计入检查。'
      : 'Only a group whose sole member is its leader, with no child groups, pages, events, albums or announcements, can be dissolved. Drafts and closed content also count.'}</p>
    {deleted ? <>
      ...
      {result && !result.canDissolve && <ul className="list-disc space-y-1 pl-5 text-sm text-[#66766f]">
        {result.blockers.map(reason => <li key={reason}>{(reasons[reason] ?? reasons.relatedRecords)[zh ? 0 : 1]}</li>)}
      </ul>}
