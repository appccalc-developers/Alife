...
const GroupApplicationsPanel = ({ groupId, language }: Props) => {
  ...
  const copy = zh ? {
    title: '入组申请', subtitle: '二维码只建立申请；组长核对本人及手机号后批准，系统再建立成员身份并生成可人工发送的激活消息。',
    ...
    noApplications: '没有符合条件的申请。', approve: '确认身份并批准', needsInfo: '补充资料', reject: '拒绝', note: '决定说明（补充资料时必填）', verified: '我已当面或通过可信渠道核对本人及这个手机号', deliveryUnavailable: '补充资料通知未能送达；状态已保留，请改用人工联系。', activationReady: '人工激活消息已生成；如已关闭复制窗口，请在成员管理生成新消息。', activationPending: '成员身份已批准，但旧邀请尚未交付；请在成员管理生成新消息。',
    previous: '上一页', next: '下一页', security: 'selector 不包含小组 ID；fragment 中的签名只在浏览器中用于安全解析。',
  } : {
    title: 'Membership applications', subtitle: 'The QR starts an application. A leader verifies the person and phone before approval; ALIFE then creates membership and a message for manual delivery.',
    ...
    noApplications: 'No applications match these filters.', approve: 'Verify identity and approve', needsInfo: 'Request information', reject: 'Reject', note: 'Decision note (required when requesting information)', verified: 'I verified this person and phone through a trusted channel', deliveryUnavailable: 'The information request was not delivered. The state is preserved for manual follow-up.', activationReady: 'A manual activation message was generated. If the copy dialog was closed, generate a new message in member management.', activationPending: 'Membership was approved, but the old invitation was not delivered. Generate a new message in member management.',
  ...
  const decide = async (application: MembershipApplication, decision: 'approved' | 'needsInfo' | 'rejected') => {
    ...
    try {
      const result = await identityAccessService.decideGroupApplication(
        ...
        decision,
        note || undefined,
        application.isContactVerified || verified[application.id] === true,
      )
