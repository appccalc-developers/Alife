...
export const eventOperationsService = {
  ...
  createTask: async (eventId: string, request: { activityId?: string | null; requireAcceptance?: boolean; stage?: string; eventOccurrenceId?: string | null; reviewerMemberId?: string | null; title: LocalizedText; description: LocalizedText; assignedMemberId?: string | null; dueUtc?: string | null; isRequired: boolean; requiresApproval: boolean; isRestricted: boolean }) => operationsChanged(eventId, (await http.post<EventTask>(`/api/events/${eventId}/tasks`, request)).data),
  updateTask: async (eventId: string, task: EventTask, status: EventTaskStatus, activityId?: string) => (await http.put<EventTask>(`/api/events/${eventId}/tasks/${task.id}`, {
    title: task.title, description: task.description, assignedMemberId: task.assignedMemberId, dueUtc: task.dueUtc,
    ...(activityId === undefined ? {} : {activityId}), status, reviewerMemberId: task.reviewerMemberId, isRequired: task.isRequired, requiresApproval: task.requiresApproval, isRestricted: task.isRestricted,
