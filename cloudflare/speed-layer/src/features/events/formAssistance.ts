...
const scenario = `Help fill ONLY the current task or registration rules form. Return proposed changed fields in form, supporting evidence per field, and a concise bilingual assistantReply (at most 1200 characters per language). Ask at most two priority questions.
...
export async function handleFormAssistance(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const access = await fetch(new URL(`/api/events/${input.eventId}/${input.scope === 'tasks' ? 'team' : 'registration-work'}`, env.API_PROXY_TARGET).toString(), { headers, redirect: 'error', signal: AbortSignal.timeout(10000) })
    ...
    if (!assistantObject(authority) || authority[input.scope === 'tasks' ? 'canManage' : 'canConfigure'] !== true) return reply({ message: 'This form is read-only. / 当前表单只读。' }, 403)
