    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
      <div className="grid gap-4 p-4 sm:p-5 md:grid-cols-2 md:items-center">
        <div className={`order-2 ${imagePosition === 'left' ? 'md:order-2' : ''}`}>
          <EditableText as="h2" value={title} fallback="Hero Section" disabled={!editable} className="text-2xl font-semibold text-slate-800 sm:text-4xl" onChange={(value) => updateLocalizedContent({ title: value, headline: value })} />
          <EditableText as="p" multiline value={body || subtitle} fallback="No hero content yet." disabled={!editable} className="mt-2 block text-base text-slate-700" onChange={(value) => updateLocalizedContent({ centerText: value, body: value, subtitle: value, subheadline: value })} />
          {linkUrl || mode === 'edit' ? (
            <a href={mode === 'render' ? linkUrl : undefined} target={mode === 'render' && linkUrl ? '_blank' : undefined} rel={mode === 'render' && linkUrl ? 'noopener noreferrer' : undefined} className="mt-4 inline-flex rounded bg-red-500 px-5 py-2 text-sm font-medium text-white shadow hover:bg-red-400" onClick={(event) => mode === 'edit' && event.preventDefault()}>
              <EditableText value={linkLabel} fallback={linkUrl || 'Read More'} disabled={!editable} className="text-sm" onChange={(value) => updateLocalizedContent({ linkLabel: value, linkText: value, ctaLabel: value })} />
            </a>
          ) : null}
        </div>
