    ...
    <section className="overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
      <div className="px-5 py-8">
        <div className="mx-auto max-w-4xl text-center">
          <EditableText as="h2" value={title} fallback="Today's Sermon" disabled={!editable} className="text-2xl font-semibold text-slate-700 sm:text-4xl" onChange={(value) => updateLocalizedContent({ title: value, headline: value })} />
          <EditableText as="p" value={subtitle} fallback="God loves us all" disabled={!editable} className="mt-1 block text-base text-slate-500 sm:text-xl" onChange={(value) => updateLocalizedContent({ subtitle: value, subheadline: value })} />
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-[1fr_1.2fr] md:items-center">
          ...
          <div className="space-y-4 text-center md:text-left">
            <EditableText as="p" multiline value={body} fallback="Sermon title and summary" disabled={!editable} className="text-lg font-semibold text-indigo-900 sm:text-2xl" onChange={(value) => updateLocalizedContent({ centerText: value, body: value })} />
