  ...
  if (!overlay) {
    return (
      <section className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-slate-700">
        <EditableText as="p" multiline value={text} fallback="No rich text content yet." disabled={!editable} className="block" onChange={(value) => updateLocalizedContent({ text: value })} />
    ...
    <section className="overflow-hidden rounded-lg border border-slate-200">
      <div className="bg-cover bg-center px-5 py-10 text-white" style={{ backgroundImage: `linear-gradient(rgba(2, 6, 23, 0.7), rgba(2, 6, 23, 0.7)), url(${bg})` }}>
        <div className="mx-auto max-w-4xl text-center">
          <EditableText as="h2" value={title} fallback="Quote of the day" disabled={!editable} className="text-2xl font-semibold sm:text-4xl" onChange={(value) => updateLocalizedContent({ title: value })} />
          <EditableText as="p" value={subtitle} fallback="God loves us all" disabled={!editable} className="mt-1 block text-base text-slate-200 sm:text-lg" onChange={(value) => updateLocalizedContent({ subtitle: value })} />
          <EditableText as="p" multiline value={text} fallback="No quote content yet." disabled={!editable} className="mt-6 block text-2xl italic leading-relaxed text-slate-100 sm:mt-8 sm:text-4xl" onChange={(value) => updateLocalizedContent({ text: value })} />
