...
Activation, group QR, and anonymous response tokens use selector/secret pairs. Raw secrets exist only in URL fragments, request bodies, and the authorized mutation response that creates them; SQL stores HMAC hashes. Anonymous submission and phone matching do not create membership. A group leader/co-leader must explicitly record in-person identity verification before approval; phone is optional and telephone verification is recorded separately; the approved transaction links or creates the member and materializes church/group membership. Possible or ambiguous existing-phone matches fail closed until an existing member is explicitly linked. For a new unregistered member without credential history, approval creates a separate durable `Active` first-activation invitation. A browser receipt can resume it without distributing its secret; legacy applicants retain the manual message. Registered members use the separately authorized recovery flow. The mutation returns the full phone and copyable bilingual message once; list APIs contain neither the message nor the raw URL, and losing the response does not roll back the human-approved membership.
...
### Browser continuation and personal recovery

[Identity access contract](identity-access.md) defines the additional cookie-bound application endpoints, nullable phone fields, and per-member recovery endpoints. Leaders/co-leaders may recover only approved ordinary members of their own open group; elevated targets require a platform administrator. Recovery rechecks authority at completion and atomically replaces old credentials, invalidating other pending registration authorizations. Recovery lasts ten minutes, accepts effective Alpha sessions for issuance, and does not invalidate existing JWT sessions. Phone verification is never inferred from Passkey registration.
