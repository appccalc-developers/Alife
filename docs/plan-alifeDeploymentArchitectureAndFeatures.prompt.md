...
- Frontend SPA/PWA served by the Cloudflare Worker in `frontend/alife-app/worker/index.ts`.
- Backend API served by Azure Functions (.NET isolated worker).
- Primary data store in Azure SQL.
- External integrations for LINE OAuth, YouTube sync, Cloudflare Images, and Gemini-backed AI sessions.
...
- Backend API deploy to Azure Functions.
...
- Deploy frontend through Wrangler and backend through the Azure Functions deployment path.
- Use preview or staging environments for release review where available.
...
- Runtime caching avoids replaying `/api/*` responses; application data freshness is handled by HTTP validators, auth, and IndexedDB-backed caches.
- Mobile install support through manifest, icons, iOS meta tags, and generated service worker registration.
