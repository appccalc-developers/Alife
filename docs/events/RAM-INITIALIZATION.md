...
4. Resolve the source colour ambiguity with the church's responsible safety authority. Explicitly set Green/Yellow/Red for **all 25 cells** and preview the matrix. Software does not infer colours or choose on behalf of the administrator.
...
授予独立的 `admin.events.manageRamPolicies` 权限，管理员还须是所选教会的已批准成员。进入“系统管理 → RAM 政策与题库”，检查双语评分定义、五类风险及题库。手册颜色有歧义，须由安全负责人逐格确认全部 25 格，再预览、保存并明确发布；系统不会代为发布。小组只继承本教会政策，无跨教会回退。未发布时仍可保存草稿。
