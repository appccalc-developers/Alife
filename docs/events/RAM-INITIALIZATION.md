...
## Activity-source migration (2026-09-15)

Additive migration `20260915101331_EventActivityPlanningSources` creates `event_activity_plans` (Event PK/FK, JSON, concurrency token, timestamp) and nullable `event_operations_tasks.activity_id`. It does not import legacy RAM activity definitions, delete history, or change signed JSON. An owner reviews legacy real activities in TEAM.WORK and explicitly saves/adopts them. Synthetic AI classification rows are not real activities. A missing source blocks background calls with `ram.sync.activityPlanRequired`; existing legacy payloads still require explicit v2 upgrade. Apply only to an approved environment before deploying dependent origin code. Deploy the matched Worker activity-key contract with origin changes; mismatched versions fail closed and preserve prior risks. No migration application or deployment is included in this change.
