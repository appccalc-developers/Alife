...
export const extractOverviewLocales = (markdown) => Object.fromEntries(localeOrder.map((locale) => {
...
export const validateOverview = (locales) => {
  ...
  for (const locale of localeOrder) {
    ...
    for (const required of [
      'EVENT-CONTRACT.md',
      'EVENT-COMPOSITION.md',
      'EVENT-PACKAGE-APPROVAL.md',
      'EVENT-WORKSPACES.md',
      'event-contract.json',
    ...
    for (const term of ['Event Plan', 'EventSeries', 'EventOccurrence']) {
  ...
  const codexSection = `<section id="codex-brief" class="section">
      <div class="section-heading">
        ...
        <div>
          ...
          <p class="muted"><span data-lang="zh-Hant">本手冊是歷史架構展示，不是現行契約或預設閱讀。依活動閱讀矩陣，只讀核心及受影響專題、模組與狀態；歷史證據按需查閱。</span><span data-lang="en">This handbook is a historical presentation, not a current contract or default reading. Follow the Event reading matrix: read the core and affected topic, module and status sections; consult historical evidence only when relevant.</span></p>
        </div>
      </div>
      <div class="contract-grid">
        <article class="contract-panel"><h3><span data-lang="zh-Hant">閱讀與規範入口</span><span data-lang="en">Reading and contract entry</span></h3><p class="muted"><a href="../AGENTS.md">Event reading matrix</a> · <a href="../EVENT-CONTRACT.md">Core and topic map</a> · <a href="../event-contract.json">Exact machine values</a></p></article>
...
// Markdown destinations also validate fragments so moved contract sections cannot
// leave bookmarks silently broken. HTML fragments may be generated by page scripts.
export const markdownAnchors = (source) => {
  const anchors = new Set([...source.matchAll(/\bid=["']([^"']+)["']/g)].map((match) => match[1]))
  ...
  for (const line of source.split(/\r?\n/)) {
    ...
    const base = heading[1].replace(/\[([^\]]+)\]\([^)]+\)/g, '$1')
      ...
      .replace(/[^\p{L}\p{M}\p{N}_\-\s]/gu, '').replace(/ /g, '-')
...
export const validateLinks = (paths) => {
  const failures = []
  ...
  for (const path of paths) {
    ...
    for (const link of links) {
      ...
      if (!existsSync(destination)) {
        failures.push(`${relative(repositoryRoot, path)} -> ${link}`)
        continue
      }
      if (fragment && extname(destination) === '.md') {
        if (!anchorCache.has(destination)) anchorCache.set(destination, markdownAnchors(readFileSync(destination, 'utf8')))
        if (!anchorCache.get(destination).has(decodeURIComponent(fragment))) failures.push(`${relative(repositoryRoot, path)} -> ${link} (missing Markdown anchor)`)
      }
    }
  }
